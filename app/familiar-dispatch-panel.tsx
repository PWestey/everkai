...
export default function FamiliarDispatchPanel({game,action,locked}:any){
 ...
 const id=chosen||owned[0]?.id,power=dispatchTeamPower(game),cleared=towerFloorOf(game);
 ...
 {owned.length<DISPATCH_TEAM?<p className="tower-hint">Contract {DISPATCH_TEAM} or more familiars to unlock. {owned.length} contracted.</p>:<>
 ...
 {run&&running?<article className="tower-floor"><h4>Area {run.area} · away</h4><p>{ready?'The team has returned.':`Time left: ${hhmm(left)}`}</p>
  ...
  :<><label htmlFor="dispatch-area">Choose an area</label><NativeSelect id="dispatch-area" value={area} onChange={e=>setArea(e.target.value)}>{DISPATCH_AREAS.map((a:any)=><NativeSelectOption key={a.id} value={String(a.id)}>Area {a.id} · {a.power.toLocaleString()} Power{dispatchUnlocked(game,a.id)?'':` · floor ${a.towerLv}`}</NativeSelectOption>)}</NativeSelect>
   <dl><div><dt>Unlocks</dt><dd>Familiar Tower floor {picked.towerLv}{cleared>=picked.towerLv?' · cleared':` · ${cleared}/${picked.towerLv}`}</dd></div><div><dt>Power needed</dt><dd>{picked.power.toLocaleString()}</dd></div><div><dt>Duration</dt><dd>{picked.hours} hours</dd></div><div><dt>Great Success</dt><dd>{full?`${greatSuccessChance(game,picked.id)}%`:'Fill the team'}</dd></div></dl>
   <p className="tower-reward">Base {picked.base.levelUp.toLocaleString()} level-up items{picked.base.classUp?` + ${picked.base.classUp} class-up`:''} · Great Success adds {picked.great.levelUp.toLocaleString()}{picked.great.classUp?` + ${picked.great.classUp} class-up`:''} and {picked.fragments.length} fragment {picked.fragments.length===1?'draw':'draws'}</p>
 ...
 <details className="rules-note"><summary>Dispatch rules and what is measured</summary>
  ...
  <p>Two things are local and flagged. The duration table does not name its unit, and no other familiar table holds one, so Everkai reads 20 as hours: at that rate the best area pays about 450 level-up items an hour against the tower’s 187, while reading it as minutes would pay 144 times the tower. And the original’s Great Success formula is not in any table — only its inputs are, so Everkai uses 30% at exactly the Power gate plus half of any surplus, capped at 100%. A Great Success also pays one draw from each of the area’s original fragment pools; fragments are kept for stars, which still cost class-up items. Cancelling forfeits the run, as in the original. Each area opens at its original Familiar Tower floor.</p></details>
 </section>;
}
