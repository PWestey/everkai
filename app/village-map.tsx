...
import {businessCost} from '@/lib/businesses.mjs';
...
export default function VillageMap({game,openBusiness,openModule,gold,locked,income,menuOpen,setMenuOpen}:any){
 ...
 {layout.buildings.map(b=>{const s=buildings[b.id];return <button type="button" key={b.id} className="village-building" style={{left:b.x+'%',top:b.y+'%',width:s.width/GROUND.width*100+'%'}} aria-label={'Visit '+b.label} aria-haspopup="dialog" onClick={()=>b.module?openModule(b.module):openBusiness(b.id)}><img src={'./assets/'+s.src} alt="" draggable={false}/><span>{b.label}<small>{b.module?'Journal':game.enterprises?.[b.id]?'Open':`Build · ${(businessCost(b.id)??0).toLocaleString()} gold`}</small></span></button>})}
 </div></div><div className="village-income"><div><small>VILLAGE GOLD</small><strong>{gold}</strong><span>+{income} / second</span></div></div><DrakenbergTown open={menu} onOpenChange={setMenu} openModule={openModule}/></section>;
}
