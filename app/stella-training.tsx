...
export default function StellaTraining({id,game,action,locked}:any){
 ...
 const ownFlat=crossover?(e&&e.level>0?p.levels[e.level-1].flat:0):(e?.flat||0);
 ...
 return <section className="school-card"><h3>{p.name} · Stella {e?e.level:'inactive'}</h3>
  ...
  <details className="rules-note"><summary>Stella effects and source boundaries</summary>
   ...
   {crossover?<p>A crossover Fellow’s Stella is a flat-Power track only: it carries Angie’s own recorded shard costs and her flat values multiplied by twelve, with no typed bonus at all. A crossover character has no Stella table, talent skills, Rarity Advance or Family Stella of its own, so this one track is sized so that a fully trained crossover Fellow stands about level with a fully trained original of the middle type — Everkai’s own balance, not the original’s. A typed bonus would be this project’s invention, so there is none. Every crossover Fellow draws on ONE shared shard pool at the same {STELLA_IDLE_PER_DAY}/day, so the supply does not grow with the roster.</p>:null}
