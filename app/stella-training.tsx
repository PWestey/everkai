...
const has=(p:any)=>{const top=p.levels.at(-1);return {
 typed:!!p.type&&top.percent>0,self:top.selfPowerBp>0,appoint:top.appointYieldBp>0,talent:top.talentLimit>0};};
export default function StellaTraining({id,game,action,locked}:any){
 ...
 const earned=[`Own flat Power +${(e?.flat||0).toLocaleString()}`];
 ...
 return <section className="school-card"><h3>{p.name} · Stella {e?e.level:'inactive'}</h3>
  ...
  <details className="rules-note"><summary>Stella effects and source boundaries</summary>
   ...
   {k.talent?<p>The talent-cap raise lets this Fellow keep buying talent levels at the same price; it grants no Aptitude on its own, and the Aptitude limit of 1,000 still applies, so a Supreme Talent Fellow will reach that limit before she uses the whole raise.</p>:null}
   ...
   {crossover?<p>A crossover Fellow’s Stella is a flat-Power track only: it carries Angie’s own recorded fragment costs and flat values with no typed bonus at all, because a crossover character has no Stella table of its own and a typed bonus would be this project’s invention. Every crossover Fellow draws on ONE shared shard pool at the same {STELLA_IDLE_PER_DAY}/day, so the supply does not grow with the roster.</p>:null}
