...
export default function StellaTraining({id,game,action,locked}:any){
 ...
 // A formerly private ladder (hero_54, hero_56, hero_190; 2026-09-19) may still hold its old fragments. They are
 // spent first on this ladder, refund to themselves, and can be converted into the village pool 1:1, one way.
 ...
 return <section className="school-card"><h3>{p.name} · Stella {e?e.level:'inactive'}</h3>
  ...
  {!e?<><Button disabled={locked} onClick={()=>run('stellaActivate')}>Activate private Stella · Free</Button>
   <details className="rules-note"><summary>About these rules</summary><p>{k.typed?`Activation grants +${stellaActivation(id)?.percent||0}% ${p.type} Power — the original's own rank-0 value.`:k.self||k.appoint?'Activation is free and grants this character’s own rank-0 values — the original’s own, not ours.':'Activation is free and grants nothing; it opens the paid ladder.'}{pooled||crossover?` One shared ${crossover?'crossover':'village'} shard pool serves every ${crossover?'crossover Fellow':'original Fellow'}, so shards spent here are shards another Fellow cannot spend.`:''}</p></details></>
  ...
  <details className="rules-note"><summary>Stella effects and source boundaries</summary>
   ...
   {ownItem?<p>This Fellow used to earn private fragments of her own at the full daily rate, which bought her Stella’s fixed Power without any choice on your part. Since 2026-09-19 nobody earns private fragments: her ladder spends the village pool like everyone’s. Fragments she already holds are spent first on her own ladder, Refund returns them to her, and Convert moves them into the village pool one for one.</p>:null}
   {pooled?<p>Every original Fellow draws on ONE shared village shard pool at the same {STELLA_IDLE_PER_DAY}/day, so the supply does not grow with the roster and a shard spent here is a shard another Fellow cannot spend. The ladder’s own cost, rank count and Power values are this character’s own, imported from the original.</p>:null}
