...
export default function CharacterScreen({person,collection,children,subtitle,stats=[],statBlock,primary,rail,info,onPrevious,onNext}: {person:any,collection:ReactNode,children:ReactNode,subtitle?:string,stats?:{label:string,value:string|number}[],statBlock?:ReactNode,primary?:ReactNode,rail?:ReactNode,info?:ReactNode,onPrevious?:()=>void,onNext?:()=>void}) {
 ...
 return <section className={'character-screen'+(hidden?' character-hidden':'')} aria-label={person.name+' character screen'}>
  ...
  <Dialog open={infoOpen} onOpenChange={setInfo}><DialogContent className="save-dialog"><DialogTitle>{person.name}</DialogTitle><DialogDescription>{person.title}</DialogDescription><p>{person.description||'No biography was recovered for this character.'}</p>{info||stats.map(stat=><p key={stat.label}>{stat.label}: {stat.value}</p>)}</DialogContent></Dialog>
 </section>;
}
