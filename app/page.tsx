...
import {SAVE_KEY,SAVE_VERSION,MAX_BUILDING_LEVEL,buildingCost,buildingRate,totalRate,fresh,decode,settle,act} from '@/lib/game.mjs';
...
export default function App(){
 ...
 return <main className="game">
  <GameResources gold={game.gold} crystals={game.crystals} income={totalRate(game)} onSettings={()=>setPanel(true)}/>
  ...
  <VillageMap menuOpen={townMenu} setMenuOpen={setTownMenu} game={game} locked={locked} income={totalRate(game).toFixed(1)} pending={fmt(game.pending)} collect={()=>action('collect')} openModule={openModule} openBusiness={(id:string)=>{setBusinessSelected(id);setBusinessVisit(n=>n+1);openModule('businesses')}}/>
  {primaryNavigation}
  <Dialog open={moduleOpen} onOpenChange={setModuleOpen}><DialogContent className={'game-modal'+(['family','fellows'].includes(tab)?' character-modal':'')} showCloseButton={false}><GameResources gold={game.gold} crystals={game.crystals} income={totalRate(game)} onSettings={()=>setPanel(true)}/><div className="modal-heading"><img src={'./assets/ui/'+(['habits','raphael','fishing','expo','stories','treasure','museum','apothecary','banquets','fountain','trading','northern','mine'].includes(tab)?'journey':tab)+'.webp'} alt=""/><div><DialogTitle>{modules.find(([id])=>id===tab)?.[1]}</DialogTitle><DialogDescription>{fmt(game.gold)} gold · {fmt(game.crystals)} crystals</DialogDescription></div><DialogClose className="village-close" aria-label="Return to village"><img src="./assets/ui/close.webp" alt=""/></DialogClose></div>{journey&&tab!=='adventure'&&<Button className="return-objective" variant="outline" onClick={()=>openModule('adventure')}>Return to next objective</Button>}<div className="modal-body" key={tab}><SystemMenus name={modules.find(([id])=>id===tab)?.[1]||''} resources={<GameResources gold={game.gold} crystals={game.crystals} income={totalRate(game)}/> }><Tabs value={tab} className="system-content">
