...
export default function App(){
 ...
 <TabsContent value="fellows">{fellowBrowse?<RosterLanding kind="Fellows" album="Hero" entries={FELLOWS} owned={game.fellows} power={(id:string)=>bondedPower(game,id)} selected={selected} status={(id:string)=>game.fellows[id]?`Lv. ${game.fellows[id].level}`:'Not joined'} onSelect={(id:string)=>{setSelected(id);setFellowBrowse(false)}}/>:<CharacterScreen person={character} onPrevious={()=>setSelected(rosterStep(rosterOrder(FELLOWS,game.fellows,(id:string)=>bondedPower(game,id)),character.id,-1))} onNext={()=>setSelected(rosterStep(rosterOrder(FELLOWS,game.fellows,(id:string)=>bondedPower(game,id)),character.id,1))} stats={owned?[{label:'Power',value:fmt(bondedPower(game,character.id))},{label:'Level',value:owned.level}]:[]} subtitle={character.occupation||character.title} collection={<Button className="collection-open" variant="outline" onClick={()=>setFellowBrowse(true)}>‹ Fellows roster</Button>}>
