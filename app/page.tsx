...
{owned?<FellowTraining game={game} id={character.id} action={action} locked={locked} workplace={workplace?.name} overview={<><p>{character.description}</p><CharacterScene key={character.id} id={character.id} locked={locked} onRead={(scene:string)=>{action('storyOpen',scene);openModule('stories')}}/> <p className="small-note">{Object.keys(game.fellows).length===FELLOWS.length?'Everyone has joined.':`${character.name} can be invited at the Recruit counter in Drakenberg.`}</p><p className="small-note">Fellows join at the Recruit counter in Drakenberg, paid for with habit-earned currency.</p><p className="next-goal">Guaranteed new arrival. No duplicates or purchases.</p></>}/>:<PanelPages key={character.id} popup personName={character.name} labels={['Welcome']}>{<><p>{character.description}</p><CharacterScene key={character.id} id={character.id} locked={locked} onRead={(scene:string)=>{action('storyOpen',scene);openModule('stories')}}/> <p className="small-note">{Object.keys(game.fellows).length===FELLOWS.length?'Everyone has joined.':`${character.name} can be invited at the Recruit counter in Drakenberg.`}</p><p className="small-note">Fellows join at the Recruit counter in Drakenberg, paid for with habit-earned currency.</p><p className="next-goal">Guaranteed new arrival. No duplicates or purchases.</p></>}</PanelPages>}
