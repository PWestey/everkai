...
export default function PanelPages({labels,children,initialPage=0,popup=false,personName='Character',selectedPage,onPageChange,variant='standard',variants}:{labels:string[],children:ReactNode,initialPage?:number,popup?:boolean,personName?:string,selectedPage?:number,onPageChange?:(page:number)=>void,variant?:PanelVariant,variants?:Record<string,PanelVariant>}){
 ...
 const [page,setLocalPage]=useState(initialPage),[open,setOpen]=useState(systemPopup&&autoOpen);const pages=Children.toArray(children),active=Math.min(selectedPage??page,Math.max(0,pages.length-1));
