...
import {magicRule,pledgeRule,magicLevel,pledgeLevel,magicBonus,pledgeOpen} from '@/lib/hero-advance.mjs';
...
export default function TalentSkillTraining({id,game,action,locked}:any){
 const list=heroTalentSkills(id) as any[],magic=magicRule(id),pledge=pledgeRule(id);if(!game.fellows[id]||(!list.length&&!magic&&!pledge))return null;
 ...
 const track=(kind:'magic'|'pledge',rule:any,level:number,label:string,note:string)=>{if(!rule)return null;return <div className="training-option" key={kind}><div><strong>{label} · Lv. {level}/{rule.max}</strong><p>{note} · {rule.cost} Stella shards a level · {shards.toLocaleString()} held</p></div><div className="business-actions">{([1,10,'max'] as const).map(amount=>{const p=advancePlan(game,id,kind,amount);return <Button key={amount} disabled={locked||!p.count} onClick={()=>kind==='magic'?action('rarityAdvance',id,amount):action('pledgeAdvance',id,amount)}>{amount==='max'?'Max':`+${amount}`} · Lv. {p.level} · {p.cost.toLocaleString()} shards</Button>})}</div></div>};
