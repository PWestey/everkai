...
import {useState} from 'react';import {Button} from '@/components/ui/button';import {NativeSelect,NativeSelectOption} from '@/components/ui/native-select';import {STAGES,stageAt,stageCost,stageReady,ladderPower,bondedPower,patrolCharges,PATROL_CEILING} from '@/lib/adventure.mjs';
// The patrol picker offers a window of recently cleared stages rather than every cleared stage: the
// ladder is 18,000 long, and a <select> with 18,000 options is unusable on a phone.
...
export default function AdventurePanel({game,action,locked,onNavigate,initialPage=0}:any){
 ...
 const chapter=next?next.chapter:STAGES[STAGES.length-1].chapter;
 ...
 <><div className="chapter-track">{[0,1,2,3,4].map(o=>chapter+o).filter(c=>c<=STAGES[STAGES.length-1].chapter).map(c=><div key={c}><strong>Chapter {c}</strong><span>{Math.max(0,Math.min(6,cleared-(c-1)*6))}/6</span></div>)}</div>
 {next?<article className="school-card"><small>{next.boss?'CHAPTER BOSS':'NEXT STAGE'} · {next.chapter}-{next.section}</small><h2>Stage {next.id.toLocaleString()} of {STAGES.length.toLocaleString()}</h2>
  {next.boss
  ...
  :<article className="school-card"><h2>Every imported chapter is cleared</h2><p>All {STAGES.length.toLocaleString()} stages of chapters 1–{STAGES[STAGES.length-1].chapter.toLocaleString()} are behind you. Patrols remain available, and the Frontier adds three local chapters.</p></article>}
