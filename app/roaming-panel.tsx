...
function EncounterCard({h}:{h:any}){const p=h.kind==='event'?null:person(h.target),goal=ROAM_FAMILY.find((f:any)=>f.id===h.target)?.bondGoal;
 return <article className={'roam-card roam-'+h.kind}>{p?<img src={'./assets/'+p.art} alt={p.name}/>:<span className="roam-event-icon" aria-hidden="true">✦</span>}<div><small>{roamLocation(h.location)?.name}</small><p>{entryText(h)}</p>{h.kind==='bond'&&goal&&<progress aria-label="Bond" max={goal} value={h.amount}/>}<small>Fame +{ROAM_FAME}</small></div></article>}
export default function RoamingPanel({game,action,locked}:any){
 ...
 return <section className="roaming-panel"><PanelPages labels={['Roam','Encounters','Records']}>
  <div className="roam-page"><div className="roam-hud"><div><small>ROAMING STAMINA</small><strong>{st.stamina}/{st.cap}</strong><span>{st.recoverAt?`Next point in ${clock(st.recoverAt-game.lastAt)}`:'Full'}</span></div><div><small>FAME</small><strong>{r.fame.toLocaleString()}</strong><span>{r.travels.toLocaleString()} roams</span></div></div>
   ...
   {last?<EncounterCard h={last}/>:<p className="small-note">Stroll through Drakenberg to meet Family members and Fellows. Uninvited Family members grow closer with each meeting and join your family when your bond is strong enough.</p>}
  </div>
  <div className="roam-grid">{ROAM_FAMILY.map((f:any)=>{const p=person(f.id),owned=!!game.family[f.id],bond=r.bonds[f.id]||0;return <article key={f.id} className={'roam-member'+(owned?' owned':'')}><img src={'./assets/'+p.art} alt=""/><div><strong>{p.name}</strong><small>{roamLocation(f.location)?.name}</small>{owned?<span>Joined · Intimacy {game.family[f.id].intimacy.toLocaleString()}</span>:<><span>Bond {bond}/{f.bondGoal}</span><progress aria-label={`Bond with ${p.name}`} max={f.bondGoal} value={bond}/></>}</div></article>})}<p className="small-note">Meeting uninvited Family members raises your bond; at the goal they join your family. Kaka (35) and Ena (50) goals match the original; other goals follow those by rarity.</p></div>
