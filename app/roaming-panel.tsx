...
export default function RoamingPanel({game,action,locked}:any){
 ...
 return <section className="roaming-panel"><PanelPages labels={['Roam','Encounters','Records']}>
  <div className="roam-page"><div className="roam-hud"><div><small>ROAMING STAMINA</small><strong>{st.stamina}/{st.cap}</strong><span>{st.recoverAt?`Next point in ${clock(st.recoverAt-game.lastAt)}`:'Full'}</span></div><div><small>FAME</small><strong>{r.fame.toLocaleString()}</strong><span>{r.travels.toLocaleString()} roams</span></div></div>
   <div className="roam-actions"><Button disabled={locked||st.stamina<1} onClick={()=>run('roamGo',{roll:Math.random()})}>Go · 1 Stamina</Button><Button variant="outline" disabled={locked||quick<2} onClick={()=>run('roamQuick',{rolls:Array.from({length:quick},()=>Math.random())})}>Quick roam ×{quick}</Button></div>
  ...
  <div className="roam-records">{r.history.length?[...r.history].reverse().slice(0,20).map((h:any)=><p key={h.id}><small>{new Date(h.at).toLocaleString([], {month:'short',day:'numeric',hour:'numeric',minute:'2-digit'})} · {roamLocation(h.location)?.name}</small>{entryText(h)}</p>):<p className="small-note item-status">No roams yet.</p>}</div>
