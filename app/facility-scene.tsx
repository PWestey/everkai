...
export default function FacilityScene({id}:{id:string}){
 ...
 if(!f)return null;
 if(backdrop)return <><div className="facility-scene" aria-hidden="true"><img key={backdrop.src} className="facility-backdrop" src={'./assets/'+backdrop.src} decoding="async" alt="" ref={i=>{if(i?.complete&&i.naturalWidth)i.classList.add('loaded')}} onLoad={e=>e.currentTarget.classList.add('loaded')}/><span className="facility-plate">{f.label}</span></div><MaturityNote id={id}/></>;
