...
import {CONSUMABLES,GEAR,MATERIALS,JOURNEY_ITEMS} from '@/lib/adventure.mjs';
const journeyIds=new Set(JOURNEY_ITEMS.map((i:any)=>i.id));
...
export default function StoragePanel({game,action,locked,onNavigate}:any){
 ...
 const entries=Object.entries(game.inventory).filter(([,count])=>typeof count==='number'&&count>0).map(([id,count])=>({id,count:count as number,name:registry[id]?.name||source[id]?.name||openingItemName(id),rarity:artifactRule(id)?.rarity||source[id]?.rarity,activity:source[id]?.isActivity===true}));
 ...
 return <section className="storage-screen" aria-label="Storage inventory">
  ...
  <Dialog open={selected!==null} onOpenChange={open=>{if(!open)setSelected(null)}}><DialogContent className="item-detail-sheet"><DialogTitle>{item?.name||source[selected||'']?.name||openingItemName(selected||'')}</DialogTitle><DialogDescription>{count.toLocaleString()} owned{(gear||source[selected||'']?.rarity)?' · Rarity '+(artifactRule(selected)?.rarity||source[selected||'']?.rarity):''}</DialogDescription>{journeyIds.has(selected||'')&&<Button onClick={()=>{setSelected(null);onNavigate?.('journey')}}>Use in Journey rewards</Button>}
