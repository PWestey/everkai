...
// Familiar Dispatch hangs off this module's own guard and action chain, so lib/game.mjs needs no
// change. The import is circular (familiar-dispatch.mjs reads familiarStats below); both sides only
// call each other from inside hoisted function declarations, never while the modules evaluate.
import {validFamiliarDispatch,dispatchAction} from './familiar-dispatch.mjs';
...
export function validFamiliars(s){
 if(!validFamiliarNodes(s)||!validFamiliarSupplies(s)||!validFamiliarDispatch(s))return false;
...
export function familiarAction(s,action,target,value){
 const nodes=familiarNodeAction(s,action,target,value)||supplyAction(s,action)||dispatchAction(s,action,target);if(nodes)return nodes;
