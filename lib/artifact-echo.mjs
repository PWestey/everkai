...
/** The Echoes a player can reach: 32 in the data, less the 5 whose named Fellow the owner deleted on
 *  2026-09-17 (hero_102, hero_113, hero_116, hero_148, hero_193). */
export const ARTIFACT_ECHOES=data.records.filter(r=>fellowById(r.fellow));
export const ARTIFACT_SUPPORTS=support.records;
/** ALL 32 rows, not just the reachable 27: validArtifactEchoes looks an enabled Echo up here, so a save
 *  that enabled one before its Fellow was deleted would otherwise be refused outright. Nothing can be
 *  enabled through a dropped row anyway -- enableArtifactEcho needs the Fellow equipped and owned, and a
 *  released Fellow is neither. */
const rules=new Map([...data.records,...ARTIFACT_SUPPORTS].map(r=>[r.item,r]));
