...
/** `PetInfo:GetPower`, transcribed (lib/familiar-combat.mjs): all NINE PetAttr rows by their CombatAdd,
 *  then the active and unlocked-passive `PetSkill.Combatcoef`. This read three attributes and no skill
 *  ratio until PetSkill was imported on 2026-09-24, so every Power on this surface was low -- 1.3x to
 *  3.5x low depending on how many skills a familiar carries. `COMBAT_ADD` is kept as the imported
 *  three-row record the dispatch data still ships; the nine-row table is the one that is read. */
export function familiarPower(id,progress){return familiarCombatPower(id,progress);}
