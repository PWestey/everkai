...
//
// Also local: the 12-floor gate mapping (ceil(TowerLv/25)), matching import-familiar-supplies.py's
// local-floor-n-is-original-floor-25n income mapping, so areas 2 and 3 both open on floor 2; and
// dispatched familiars are not withheld from the Familiar Tower team.
...
/** An area is offered once the local tower floor it maps to has been cleared. */
export const dispatchUnlocked=(s,areaId)=>{const a=dispatchArea(areaId);return !!a&&(s.familiarTower?.cleared||0)>=a.floor;};
...
export function dispatchAction(s,action,target){
 ...
 if(action==='dispatchStart'){
  ...
  if(!dispatchUnlocked(s,area.id))return fail(`Unlocks at floor ${area.floor} in the Familiar Tower.`);
