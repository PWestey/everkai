 ...
 *
 *  WHAT THE ORIGINAL ACTUALLY GRANTS, per rank, and what Everkai models:
 *    self  atk extradd    126 heroes   the owner's own flat Power        MODELLED (`flat`)
 *    country atk percent    4 heroes   +N% to every Fellow of one TYPE   MODELLED (`percent`)
 *    self  atk percent    116 heroes   +N% to the owner alone            NOT modelled -- see below
 *    all   appoint percent  57 heroes  every Fellow's appointment yield  NOT modelled
 *    self  talentLvLimit    57 heroes  +100 to the owner's talent cap    NOT modelled
 *    bond  atk% / talent   114 ids     a named hero group's Power        NOT modelled
 *  The unmodelled columns are not lost: `unmodelledMax` in the data file carries each one's maxed value
 *  per hero, so tests/hero-spirit.test.mjs can price them without another research pass. Measured cost
 *  of adding the biggest one (self percent, up to +1350%): the flag-off ceiling goes 15,622,961 ->
 *  26,936,296, i.e. 4.47x -> 7.70x of the original's live-save reference, against the ~4x the owner
 *  accepted. That is why it is deferred rather than dropped.
 *
...
function profile(id,src,{itemId,name,type,flatOnly=false}){
 ...
 // `flatOnly` drops the typed percent and nothing else. It is used for LOCAL DECISION 2 only: the
 ...
 // times over, since the type sum adds every owner. The flat column is untouched.
 const pct=r=>flatOnly?0:r.percent/100;
 return Object.freeze({
  id,name,type,itemId,flatOnly,
  levels:Object.freeze(paid.map(r=>Object.freeze({level:r.rank,cost:r.cost,itemId,flat:r.flat,percent:pct(r)}))),
  activation:Object.freeze({cost:0,flat:src.ranks[0].flat,percent:pct(src.ranks[0])}),
  source:`lib/hero-spirit-data.json hero_${src.heroId}${flatOnly?' (flat column only)':''}`,
