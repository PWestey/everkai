...
export function validAdventure(s){const int=(n,max)=>Number.isInteger(n)&&n>=0&&n<=max;
 ...
 return (s.shopPearls===undefined||int(s.shopPearls,1e12))&&validOriginalProgression(s)&&validTrainingCosts(s)&&validFishing(s)&&validInsight(s)&&validTalents(s)&&validArtifacts(s)&&Object.entries(s.fellows).every(([id,f])=>int(f.aptitude,1000)&&f.aptitude>=10&&int(f.skill,20)&&int(f.breaks,13)&&(f.stars===undefined||int(f.stars,STAR_CAP))&&f.level<=fellowCap(f,s,id)&&(f.gear===null||(s.version>=10?GEAR:ORIGINAL_GEAR).some(g=>g.id===f.gear)));
}
export function adventureAction(s,action,target,value){const fail=error=>({state:s,error});const bounded=n=>Math.min(1e9,n);const count=n=>Math.min(1e6,n);
 ...
 if(action==='battle'||action==='patrol'){const stage=STAGES.find(x=>x.id===target);if(!stage||(action==='battle'?stage.id!==s.adventure.cleared+1:stage.id>s.adventure.cleared))return fail('Choose the next stage or a cleared patrol.');
  ...
  return {state:{...s,gold:Math.min(MAX_GOLD,s.gold-cost+(action==='battle'?stage.gold:cost)),fellowXP:bounded(s.fellowXP+xp),crystals:bounded(s.crystals+(action==='battle'?stage.crystals:0)),inventory,...(fountain?{fountain}:{}),adventure:{...s.adventure,cleared:action==='battle'?stage.id:s.adventure.cleared,patrols:bounded(s.adventure.patrols+(action==='patrol'?1:0)),...(action==='patrol'?{patrolAt:spendPatrol(s,s.lastAt)}:{}),lastBattle}},message:action==='battle'?`Stage ${stage.id} cleared! +${stage.xp} EXP, ${stage.crystals} crystals, ${STAGE_BOTTLES} Fairy Bottles and materials.`:`Patrol complete: +${xp} EXP. Entry gold returned. ${patrolCharges({...s,adventure:{...s.adventure,patrolAt:spendPatrol(s,s.lastAt)}})} patrol supplies left.`}}
 return null;
}
