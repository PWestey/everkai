...
import {STAGES,stageAt,stageCost,stageReady,openingBossReady} from './stage-ladder.mjs';
export {STAGES,stageAt,stageCost,stageReady};
...
export function validAdventure(s){const int=(n,max)=>Number.isInteger(n)&&n>=0&&n<=max;
 ...
 const b=s.adventure.lastBattle;if(b!==null&&(!b||!int(b.stage,STAGES.length)||b.stage<1||!int(b.power,1e18)||typeof b.won!=='boolean'||!['battle','patrol'].includes(b.kind)))return false;
...
export function adventureAction(s,action,target,value){const fail=error=>({state:s,error});const bounded=n=>Math.min(1e9,n);
 ...
 if(action==='battle'||action==='patrol'){const stage=stageAt(target);if(!stage||(action==='battle'?stage.id!==s.adventure.cleared+1:stage.id>s.adventure.cleared))return fail('Choose the next stage or a cleared patrol.');
