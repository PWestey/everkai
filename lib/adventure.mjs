...
export function adventureAction(s,action,target,value){const fail=error=>({state:s,error});const bounded=n=>Math.min(1e9,n);const count=n=>Math.min(1e6,n);
 ...
 if(action==='claimAllGear'){const inventory={...s.inventory};for(const g of GEAR)inventory[g.id]=Math.min(1e6,inventory[g.id]+1);return {state:{...s,inventory},message:'Sandbox: one of every artifact added, up to bag limits.'}}
 if(action==='claimGear'){const item=GEAR.find(i=>i.id===target);if(!item||item.price!==null)return fail('Choose equipment offered for sandbox use.');if(s.inventory[target]>=1e6)return fail('Your bag is full for this item.');return {state:{...s,inventory:{...s.inventory,[target]:s.inventory[target]+1}},message:`${item.name} added for sandbox use.`}}
