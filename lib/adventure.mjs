...
export function aptitudeTrainingPlan(s,id,amount='max'){
 const f=s.fellows[id];const count=!f||![1,5,'max'].includes(amount)?0:Math.max(0,Math.min(amount==='max'?APTITUDE_CAP:amount,APTITUDE_CAP-f.aptitude,s.inventory.Item_Talent_Hero_1));return {count,cost:count,aptitude:(f?.aptitude||0)+count};
}
