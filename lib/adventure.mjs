...
export function powerParts(s,id){
 ...
 const coefpercent={};
 const percent={stars:fellowStars(f)*STAR_POWER_BP,skill:(f.skill||0)*SKILL_POWER_BP,bonds:Math.round((bondFactor(s,id)-1)*POWER_BP),family:Math.round(b.percent*POWER_BP),museum:bp(m.basicPowerPercent),fishing:bp(fish.percent),echo:bp(echo.percent),stella:bp(st.percent),familiar:bp(pet.percent),familyStella:fs.percent,familyPair:Math.round(pair.percent*POWER_BP),quench:quenchPercent(f)};
 const flat={family:b.flat,fishing:fish.flat,stella:st.flat,elixir:elixirPower(s,id),familiar:pet.flat,museum:m.flat,familyPair:pair.flat};
