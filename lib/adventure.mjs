...
import {handbookPowerBP} from './familiar-handbook.mjs';
...
import {talentSkillParts} from './talent-skills.mjs';
import {familyStellaParts,familyPairBlessing} from './family-stella-power.mjs';
import {quenchPercent} from './quench.mjs';
import {starParts,starHaloParts,originParts} from './hero-stars.mjs';
import {originLevel} from './hero-advance.mjs';
...
export const MATERIALS=[{id:'Item_Talent_Hero_1',name:'Skill Pearl',price:200,detail:'Local effect: +1 Fellow Aptitude or one talent-skill Aptitude. The price doubles every 1,000 pearls bought; the shop sells at most 150 a day.'},{id:'local_skill_scroll',name:'Skill Scroll',price:300,detail:'Improve a Fellow skill'},{id:'local_limit_token',name:'Local Limit-Break Token',price:1000,detail:'Raise a Fellow level cap'}];
...
// In APK growth the cap is the BETTER of the earned crystal quality and the tier this Fellow's own limit
// breaks already grant (lib/original-progression.mjs sourceTier), so the two modes agree on `breaks` and
// switching can never take a level limit away. `f` is passed through because Refund all asks what the cap
// WOULD be with one fewer break (lib/fellow-reset.mjs) -- reading s.fellows[id] there would answer the
// question that was not asked and hand back a token the kept level still needs.
export const fellowCap=(f,s=null,id=null)=>originalProgression(s)?sourceCap(s,id,f):qualityRule(Math.min(14,(f.breaks||0)+1)).cap;
...
// The batch sizes the Upgrade screen offers. 10 and 100 joined 1, 5 and max on 2026-09-22 when the
// screen was rebuilt to the original's `Quick | x1 | x10 | x100` selector (docs/fellow-screen-specs/
// 03-upgrade.md). NOTHING about a level's price or effect changes: `train x10` walks the same loop ten
// single levels walk, so a save trained in batches is a save trained one level at a time. 5 is kept
// because saves, tests and lib/fellow-reset.mjs already name it.
export const TRAIN_AMOUNTS=Object.freeze([1,5,10,100,'max']);
/** @param {number|string} [amount] */
export function levelTrainingPlan(s,id,amount='max'){
 const f=s.fellows[id];if(!f||sourceTraining(s)&&s.trainingCosts.receipts.length>=10000||!TRAIN_AMOUNTS.includes(amount))return {count:0,cost:0,level:f?.level||0};
...
export function aptitudeTrainingPlan(s,id,amount='max'){
 const f=s.fellows[id];const count=!f||!isBatchAmount(amount)?0:Math.max(0,Math.min(amount==='max'?PEARL_APTITUDE_CAP:amount,PEARL_APTITUDE_CAP-f.aptitude,s.inventory.Item_Talent_Hero_1));return {count,cost:count,aptitude:(f?.aptitude||0)+count};
}
...
// price doubles every PEARL_PRICE_DOUBLING pearls bought over the save's life (counted in s.shopPearls).
//
// TIGHTENED 2026-09-18 from 2,500 to 1,000 (docs/power-sources-import-spec.md 7 question 2, owner's answer:
// pearls "were harder to come by", and Power grew mostly WITHOUT pearl-bought Aptitude). Measured, both
// counts from each game's own records (rule 1: a count against a count):
//   * the original: no permanent pearl faucet at all. A census of every config table that pays
//     Item_Talent_Hero_1 finds only event rank/battle-pass rewards (split_reward/reward_Ns 1s..100s,
//     65-73k each across all events), VIP/gift codes and event minigames; no shop, no stage, no daily task.
//     The owner's few-weeks account (paid purchases included) holds ~10,860 in total (spec 1.3).
//   * Everkai: the day-30 pacing sim bought 24,634 from this shop -- 99.5% of every pearl it received
//     (Frontier, Expo and Workshop paid the other ~116) -- 2.3x the owner's paid-inclusive total.
// The sim buys up to roughly the same marginal PRICE whatever the doubling is (it spends by return on gold),
// so pearls bought scale with the doubling: 1,000 puts day 30 near 10k, the owner's own figure as an
// upper bound. Nothing stored is priced from this (s.shopPearls is a count), so no save moves (rule 12).
export const PEARL_ID='Item_Talent_Hero_1',PEARL_BASE_PRICE=200,PEARL_PRICE_DOUBLING=1000;
export const pearlPrice=bought=>Math.min(MAX_GOLD,Math.round(PEARL_BASE_PRICE*2**(bought/PEARL_PRICE_DOUBLING)));
// A DAILY LIMIT, 360 pearls (added with the account-floor step, 2026-09-18). MEASURED, then chosen: the
// price ladder alone does not bound pearls once they have somewhere to go. With talent skills to absorb them,
// the step-1 build's day-30 pacing sim bought 32,695 at the 1,000-pearl doubling -- MORE than the 24,634 it
// bought at 2,500 with nowhere past Aptitude 1,000 to put them -- because income compounds and the sim buys
// to a price measured in minutes of income, whatever the ladder. The original has no pearl shop and no
// permanent pearl source at all (every config table that pays Item_Talent_Hero_1 is an event); the owner's
// paid-inclusive few-weeks account holds ~10,860, i.e. 360 a day over 30 days, as an UPPER bound. So the
// shop sells at most that many a day. Stored as {day, bought}; a save without it has bought none today.
// OWNER ANSWER 2026-09-19 (rule 11): "as a free player, a normal day brought about 150 Skill Pearls". The limit
// is that F2P rate; 360 (the paid-inclusive upper bound above) was the placeholder until he answered. Save-neutral:
// shopPearlDay is validated against 1e6, not this limit, so a save that bought 360 today just has no room left.
export const PEARL_DAILY_LIMIT=150;
const dayKey=now=>{const d=new Date(now),p=n=>String(n).padStart(2,'0');return `${d.getFullYear()}-${p(d.getMonth()+1)}-${p(d.getDate())}`;};
export const pearlsToday=s=>s.shopPearlDay?.day===dayKey(s.lastAt)?s.shopPearlDay.bought:0;
export function supplyPurchasePlan(s,id,amount=1){const item=EXTRA_ITEMS.find(i=>i.id===id);if(!item||item.price==null||![1,5,25].includes(amount))return {count:0,cost:0,currency:'gold'};if(id===PEARL_ID){const bought=s.shopPearls||0,room=Math.max(0,Math.min(amount,1e6-s.inventory[id],PEARL_DAILY_LIMIT-pearlsToday(s)));let count=0,cost=0;while(count<room&&cost+pearlPrice(bought+count)<=s.gold){cost+=pearlPrice(bought+count);count++;}return {count,cost,currency:'gold'};}const currency=GEAR.some(g=>g.id===id)?'crystals':'gold',count=Math.max(0,Math.min(amount,1e6-s.inventory[id],Math.floor(s[currency]/item.price)));return {count,cost:count*item.price,currency};}
...
export function powerParts(s,id){
 const f=s.fellows[id],b=blessingPower(s,id),m=museumBonus(s,id),pet=familiarBonus(s,id),fish=fishingBonuses(s,id),echo=artifactEchoBonus(s,id),st=stellaBonus(s,id);
 // Skill Aptitude (docs/power-sources-import-spec.md 1): every talent skill but Base_N and Insight I, the
 // intimacy skill, the Stella self/bond talent halos, Rarity Advance's talentBonus and its stage's initial
 // talent. All derived (lib/talent-skills.mjs); none of it is in f.aptitude.
 const ts=talentSkillParts(s,id),fs=familyStellaParts(s,id),pair=familyPairBlessing(s,id);
 // Stars pay HeroStar's own row, gated by level (step 4); every owned hero's star halos broadcast; Origin Boost.
 const star=starParts(f),halo=starHaloParts(s,id),origin=originParts(id,originLevel(s,id));
 const talent={record:f.aptitude,hero:sourceAptitudeBonus(s,id),gear:GEAR.find(g=>g.id===f.gear)?.aptitude||0,artifact:artifactBonus(f),family:specialAptitude(s,id),museum:m.aptitude,fishing:fish.aptitude,echo:echo.aptitude,familiar:pet.aptitude,skills:ts.skills,intimacy:ts.intimacy,stellaTalent:ts.stella,stellaBond:ts.stellaBond,rarity:ts.rarity,stage:ts.stage,familyStella:fs.talent,starHalo:halo.talent,origin:origin.talent};
 const coefpercent={starHalo:halo.coef,origin:origin.coef};
 const percent={stars:star.percent,starHalo:halo.percent,origin:origin.percent,skill:(f.skill||0)*SKILL_POWER_BP,bonds:Math.round((bondFactor(s,id)-1)*POWER_BP),family:Math.round(b.percent*POWER_BP),museum:bp(m.basicPowerPercent),fishing:bp(fish.percent),echo:bp(echo.percent),stella:bp(st.percent),familiar:bp(pet.percent),familyStella:fs.percent,familyPair:Math.round(pair.percent*POWER_BP),quench:quenchPercent(f),compendium:handbookPowerBP(s,id)};
 const flat={family:b.flat,fishing:fish.flat,stella:st.flat,elixir:elixirPower(s,id),familiar:pet.flat,museum:m.flat,familyPair:pair.flat,stars:star.flat};
...
export function validAdventure(s){const int=(n,max)=>Number.isInteger(n)&&n>=0&&n<=max;
 ...
 if(s.shopPearlDay!==undefined&&(!s.shopPearlDay||typeof s.shopPearlDay.day!=='string'||s.shopPearlDay.day.length>10||!int(s.shopPearlDay.bought,1e6)||Object.keys(s.shopPearlDay).length!==2))return false;
...
export function adventureAction(s,action,target,value){const fail=error=>({state:s,error});const bounded=n=>Math.min(1e9,n);
 ...
 if(action==='buySupply'){const amount=value??1;if(![1,5,25].includes(amount))return fail('Choose a purchase amount.');const plan=supplyPurchasePlan(s,target,amount);if(!plan.count)return fail(target===PEARL_ID&&pearlsToday(s)>=PEARL_DAILY_LIMIT?`The shop sells ${PEARL_DAILY_LIMIT} Skill Pearls a day. More tomorrow.`:'This supply is unavailable, your bag is full, or you need more currency.');return {state:{...s,[plan.currency]:s[plan.currency]-plan.cost,...(target===PEARL_ID?{shopPearls:(s.shopPearls||0)+plan.count,shopPearlDay:{day:dayKey(s.lastAt),bought:pearlsToday(s)+plan.count}}:{}),inventory:{...s.inventory,[target]:s.inventory[target]+plan.count}},message:`${plan.count} supplies purchased for ${plan.cost} ${plan.currency}.`};}
