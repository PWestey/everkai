...
import {hasCrossoverAbilities,crossoverInsightRule} from './crossover-abilities.mjs';
...
/** Eligibility is entirely a function of `type`: the five rules are numerically identical and 159 of
 *  159 originals qualify. A crossover Fellow reads the rule for its OWN type rather than its template's
 *  -- the same rule, since the template is its type's anchor, but now for its own reason. */
export const insightRule=id=>{if(hasCrossoverAbilities(id))return crossoverInsightRule(id);const r=rules.get(id);return r&&fellowById(id)?.type===r.type?r:null};
...
export function insightTrainingPlan(s,id,amount=1){
 ...
 const count=!r||!f||!isBatchAmount(amount)?0:Math.max(0,Math.min(amount==='max'?r.supportedLevels:amount,r.supportedLevels-level,Math.floor(balance/r.cost),Math.floor((APTITUDE_CAP-f.aptitude)/r.aptitude)));
...
export function insightAction(s,action,id,value=1){
 ...
 if(!isBatchAmount(value))return fail('Choose an Insight training amount.');
 if(level>=r.supportedLevels)return fail('The supported Insight levels are complete.');
 if(f.aptitude+r.aptitude>APTITUDE_CAP)return fail('This Fellow has reached the sandbox Aptitude limit.');
