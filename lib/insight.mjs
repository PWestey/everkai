...
export function insightTrainingPlan(s,id,amount=1){
 ...
 const count=!r||!f||![1,5,'max'].includes(amount)?0:Math.max(0,Math.min(amount==='max'?r.supportedLevels:amount,r.supportedLevels-level,Math.floor(balance/r.cost),Math.floor((APTITUDE_CAP-f.aptitude)/r.aptitude)));
