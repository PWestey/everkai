...
/** What a character costs: set members take their own insignia, everyone else takes Acquaint Stones. */
export const SUMMON_COSTS={SR:{stones:1},SSR:{stones:2},'SSR+':{stones:3},UR:{insignias:2},'UR*':{insignias:2},set:{insignias:1}};
...
export const summonCost=(rarity,inSet=false)=>inSet?SUMMON_COSTS.set:SUMMON_COSTS[String(rarity||'').split(' ->')[0].trim()]||SUMMON_COSTS.SSR;
...
export function summonAction(s,action,target,value){
 ...
 if(action==='summonClaimDay'){
  ...
  return {state:{...s,summon:next},message:perfect?`Perfect day · ${fragments} Acquaint Stone Fragments, 1 insignia fragment, ${PERFECT_DAY_SHARDS} star shards`:`${done}/${due} dailies · ${fragments} Acquaint Stone Fragments`};
 }
