...
import {starRosterGate} from './hero-stars.mjs';
import roster from './public-roster.json' with {type:'json'};
import {additionById,isAddition} from './everkai-additions.mjs';
import {unlockEvent} from './events.mjs';
...
// The counter offers 244 characters today (and more with the crossover additions), so a 200-entry
// receipt log told a completionist "Recruitment record is full." before they finished the roster.
// The log is only a record, so the cap exists to bound the save, not to limit recruiting: it is now
// comfortably above every character the counter can sell.
export const RECRUIT_RECEIPTS=600;
...
/** Everkai additions are not for sale, the same way the 22 rank-up Fellows are not: the only route in
 *  is the storyline arc that hands them over (lib/events.mjs, docs/crossover-plan.md order of work 3).
 *  Measured before this: every crossover character starts at rarity N, SUMMON_COSTS.N is 3 fragments
 *  and habits pay up to 10 a day, so the counter would have handed over all 163 in 49 days at three a
 *  day -- the "bought cheaply" the owner ruled out. recruitOffers filters on recruitPrice being truthy,
 *  so null drops them out of the counter with no other change, and validSummon never re-derives a
 *  price, so a flag-on save holding a receipt from before this keeps validating (verified in
 *  tests/everkai-additions.test.mjs). */
export const recruitPrice=id=>RANK_FELLOWS.has(id)||isAddition(id)?null:FREE_ROSTER.has(id)?FREE_PRICE:summonCost(recruitRarity(id));
...
export function summonAction(s,action,target,value){
 ...
 if(action==='summonRecruit'){
  ...
  // An addition has no price at all (see recruitPrice). Say where they DO come from rather than
  // "no price is recorded", which reads like missing data instead of a deliberate route.
  if(isAddition(person.id)){const arc=unlockEvent(person.id);
   return fail(arc?`${person.name} joins by playing ${arc.name}, not at the counter.`:`${person.name} is not invited at the counter.`)}
 ...
 if(action==='summonStar'){
  ...
  // THE ORIGINAL'S OWN PREREQUISITE (2026-09-22, HeroStar.needHeroStarCount): stars 4, 5 and 6 need
  // 15 / 20 / 25 Fellows already at *3 / *4 / *5 on the account. It gates the PURCHASE only -- a
  // stored star is never refused and never stops paying (lib/hero-stars.mjs starRosterGate).
  const gate=starRosterGate(s,target);
  if(gate&&!gate.met)return fail(`Awaken ${gate.count-gate.have} more Fellows to ${gate.star}★ first (${gate.have}/${gate.count}).`);
