...
export const CURRENCY_NAMES={stoneFragments:'Acquaint Stone Fragments',stones:'Acquaint Stones',insignias:'insignias'};
...
export const recruitPrice=id=>summonCost(recruitRarity(id));
...
export function summonAction(s,action,target,value){
 if(!['summonClaimDay','summonClaimWeek','summonForge','summonRecruit'].includes(action))return null;
 ...
 if(action==='summonRecruit'){
  ...
  return {state:{...s,summon:next,[person.kind]:{...s[person.kind],[person.id]:joined}},
   message:`${person.name} joined for ${amount} ${CURRENCY_NAMES[currency]}.`,
