...
export function northernAction(s,action,target,value){if(!['northStart','northTile','northAttack','northNext','northFinish','northTrain','northExchange'].includes(action))return null;const old=northern(s),fail=error=>({state:s,error});if(value?.seq!==old.seq||old.seq>=1e9)return fail('The expedition changed. Use current controls.');const n={...old,...northernSupplies(s),seq:old.seq+1},done=message=>({state:{...s,northern:n},message});
