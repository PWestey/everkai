...
import {validInn,settleInn,settleInnStamina,innAction} from './inn.mjs';
...
import {FELLOWS,BUILDINGS,FAMILY,GIFTS,fellowById,REMOVED} from './catalog.mjs';
...
export const startingSave=(now=Date.now())=>{const base=fresh(now),r=starterRoster();
 return {...base,habits:starterHabits(now),fellows:{hero_1:newFellow(),...r.fellows},buildings:{...base.buildings,fish:{...base.buildings.fish,fellow:'hero_1'}},adventure:{...base.adventure,party:['hero_1']},family:{...base.family,...r.family}}};
...
function repairSave(s){
 ...
 s=releaseRemoved(s);
 const family=stripRemovedRecipients(s.family);if(family!==s.family)s={...s,family};
...
if(!valid(s))throw Error('This is not a compatible village save.');return s}
...
export function settle(state,now){if(!valid(state)||!number(now))throw Error('Invalid village state');const elapsed=Math.min(MAX_AWAY_MS,Math.max(0,now-state.lastAt));return settleInnStamina(settleHabits(settleApothecary(settleWorkshop(settleInn({...state,school:{...state.school,points:Math.min(EDUCATION_CAP,state.school.points+elapsed/EDUCATION_RECOVERY_MS)},energy:Math.min(energyCap(state),state.energy+elapsed/ENERGY_RECOVERY_MS),pending:Math.min(MAX_GOLD,state.pending+accrue(state,state.lastAt,elapsed)),lastAt:Math.max(now,state.lastAt)},Math.max(now,state.lastAt)),Math.max(now,state.lastAt)),Math.max(now,state.lastAt)),now),state.lastAt,now)}
