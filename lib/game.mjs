import {validHabits,settleHabits,habitAction} from './habits.mjs';
...
export function settle(state,now){if(!valid(state)||!number(now))throw Error('Invalid village state');const elapsed=Math.min(MAX_AWAY_MS,Math.max(0,now-state.lastAt));return settleHabits(settleApothecary(settleWorkshop(settleInn({...state,school:{...state.school,points:Math.min(EDUCATION_CAP,state.school.points+elapsed/EDUCATION_RECOVERY_MS)},energy:Math.min(energyCap(state),state.energy+elapsed/ENERGY_RECOVERY_MS),pending:Math.min(1e9,state.pending+elapsed/1000*totalRate(state)),lastAt:Math.max(now,state.lastAt)},Math.max(now,state.lastAt)),Math.max(now,state.lastAt)),Math.max(now,state.lastAt)),now)}
