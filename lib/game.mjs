...
function actCore(state,action,now=Date.now(),target=null,value=null){
 ...
 if(action==='autoDate'){
  ...
  // A PER-DATE REPORT, not an aggregate (docs/family-screen-specs/09-auto-date.md). Everkai applied
  // every date's rewards and updated the numbers in place, so the run's only trace was one sentence
  // -- "5 dates completed: +7,105 Blessing Points" -- and the CG a date unlocked was announced
  // nowhere at all. The original reports each date on its own card and then announces each new CG.
  // The report rides on the RESULT, never the state: nothing here is stored, so no save is affected.
  const before=Object.values(s.family).reduce((sum,f)=>sum+f.points,0);let next=s;const report=[];
  for(const roll of rolls){
   const pointsBefore=Object.values(next.family).reduce((sum,f)=>sum+f.points,0);
   const result=act(next,'date',s.lastAt,null,roll);if('error' in result)return fail(result.error);
   const id=result.welcomed;
   report.push({id,name:familyById(id)?.name??id,
    points:Object.values(result.state.family).reduce((sum,f)=>sum+f.points,0)-pointsBefore,
    picture:result.pictureDiscovered??null});
   next=result.state;
  }
  const gained=Object.values(next.family).reduce((sum,f)=>sum+f.points,0)-before;
  return {state:next,report,message:`${count} dates completed: +${gained.toLocaleString()} Blessing Points.`};
 }
