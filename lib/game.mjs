...
import {validTower,towerAction,migrateTower} from './familiar-tower.mjs';
...
function repairSave(s){
 ...
 // FAMILIAR TOWER POLICY 1 -> 2 (2026-09-16). The 12-floor sandbox becomes the original 300 floors;
 // local floor n paid original floor 25n's income, so it is stored as floor 25n and the sandbox record is
 // kept under `legacy` (lib/familiar-tower.mjs migrateTower). Inert on a policy-2 or absent tower.
 if(s.familiarTower&&typeof s.familiarTower==='object'&&s.familiarTower.policyVersion===1)s={...s,familiarTower:migrateTower(s.familiarTower)};
...
function reconcileRecipients(family){
 ...
 const heal=(stored,table)=>{
  ...
  if(stored.some(x=>!known.has(x)&&originalCharacter(x)))return stored;
  return [...table];
 };
 let changed=false;
...
export const VALIDATORS={validVillageEvents,validAchievements,validEvents,validHelper,validSupplyClaim,validHabits,validWardrobe,validFamilyGallery,validOpening,validMine,validStella,validNorthern,validRoaming,validSummon,validTradingPost,validStageEvent,validTonics,validElixirs,validFountain,validBanquets,validApothecary,validTreasure,validV4,validStory,validExpo,validTower,validFamiliarExplore,validFrontier,validInn,validWorkshop,validFarm,validHireCards,validBusinesses,validMuseum,validBlessings,validFamiliars,validStage,validAdventure,validBonds,validFathoms,validFamilyTrips};
...
export function buildingRate(s,id){const b=s.buildings[id];if(!b?.fellow)return 0;return rate(s.fellows[b.fellow].level)*(1+.2*(b.level-1))*(1+familyBonus(s))*fellowFactor(s.fellows[b.fellow])*bondFactor(s,b.fellow)*(1+potionYield(s))}
