import {validHabits,settleHabits,habitAction,habitEarnings} from './habits.mjs';
...
export function newJourney(state,now=Date.now()){const base=fresh(now);return state?.habits?{...base,habits:{...state.habits}}:base}
...
function actCore(state,action,now=Date.now(),target=null,value=null){
 ...
 const north=northernAction(s,action,target,value);if(north)return north;const roam=roamingAction(s,action,target,value);if(roam)return roam;
