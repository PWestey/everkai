 ...
 *
 *  It returns FEWER than ten when the franchise has fewer than ten crossover Fellows in the data, which
 *  is the case today: 2 of the planned 133 Fellow rows ship. A list can only name characters that
 *  exist, because `validBlessings` requires every recipient to resolve through `fellowById`. As Fellow
 *  rows land, re-running scripts/crossover/write-family-recipients.mjs regrows the lists; nothing stored
 *  in a save depends on them (no `apkBlessings` record is ever written for an addition), so regrowing
 *  them cannot refuse an older save -- which is exactly why the lists were allowed to be short. */
