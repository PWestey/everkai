 ...
 *  track in lib/crossover-stella.mjs (Angie's own cost/flat columns, percent 0, one pool for the whole
...
// The switch is remembered, because the home-screen app always opens without a query string: once the
// page has been opened with ?crossover=1 it stays on, and ?crossover=0 turns it off again. The memory is
// a device preference, not save data, so it can never change what a save holds. Node has neither
// `location` nor `localStorage`, so tests and sims still see exactly the original catalogue.
const STORE_KEY='everkai-crossover';
const deviceStorage=()=>{try{return typeof localStorage==='undefined'?null:localStorage}catch{return null}};
export function crossoverEnabled(search=typeof location==='undefined'?'':location.search,storage=deviceStorage()){
 const asked=new URLSearchParams(search).get(ADDITIONS_FLAG);
 try{
  if(asked==='1'){storage?.setItem(STORE_KEY,'1');return true}
  if(asked==='0'){storage?.removeItem(STORE_KEY);return false}
