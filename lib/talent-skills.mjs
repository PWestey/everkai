...
import {magicRule,pledgeRule,magicLevel,pledgeLevel,magicBonus,magicStage,pledgeOpen,heroAdvanceState} from './hero-advance.mjs';
...
/** Stars as the original counts them: Everkai sells 7, HeroStar has 6 (spec 7 question 7: ★7 clamps to ★6). */
const stars=f=>Math.min(6,f?.stars||0);
...
export const talentSkillLimit=(s,id)=>stellaTalentLimit(s,id);
...
export function validTalentSkills(s){
 for(const [id,f] of Object.entries(s.fellows)){const t=f.talentSkills;if(t===undefined)continue;
  ...
  const own=new Set(heroTalentSkills(id).map(x=>x.skill)),bound=maxLimit.get(id)||0;
...
export function talentSkillAction(s,action,target,value){
 ...
 if(action==='rarityAdvance'||action==='pledgeAdvance')return advanceAction(s,action,target,value);
 return null;
}
...
export function advancePlan(s,id,track,amount=1){
 const rule=track==='magic'?magicRule(id):pledgeRule(id),level=track==='magic'?magicLevel(s,id):pledgeLevel(s,id);
...
function advanceAction(s,action,target,amount=1){
 const fail=error=>({state:s,error}),track=action==='rarityAdvance'?'magic':'pledge';
 if(amount==null)amount=1;
 if(!s.fellows[target]||!(track==='magic'?magicRule(target):pledgeRule(target)))return fail(track==='magic'?'This Fellow has no Rarity Advance in the original.':'This Fellow has no Pledge in the original.');
 ...
 const p=advancePlan(s,target,track,amount);if(!p.count)return fail(p.level>=(track==='magic'?magicRule(target).max:pledgeRule(target).max)?'Already at the top level.':'Not enough Stella shards.');
 ...
 return {state:{...s,stella:{...st,stock:{...st.stock,[SPIRIT_SHARD_ITEM]:st.stock[SPIRIT_SHARD_ITEM]-p.cost}},heroAdvance:{policyVersion:1,fellows:{...t.fellows,[target]:{...t.fellows[target],[track]:p.level}}}},
  message:`${track==='magic'?'Rarity Advance':'Pledge'} Lv. ${p.level} for ${p.cost.toLocaleString()} Stella shards.`};
}
