...
export function talentSkillPlan(s,id,skill,amount=1){
 ...
 const f=s.fellows[id],r=TALENT_SKILLS[skill],w=wallet(s,id,skill);if(!f||!r||!w||!isBatchAmount(amount)||!talentSkillUnlocked(s,id,skill))return none;
...
export function talentSkillAction(s,action,target,value){
 if(action==='trainTalentSkill'){
  ...
  if(!isBatchAmount(amount))return fail('Choose a talent skill training amount.');
...
export function advancePlan(s,id,track,amount=1){
 ...
 const none={count:0,cost:0,level};if(!rule||!s.fellows[id]||!isBatchAmount(amount))return none;
