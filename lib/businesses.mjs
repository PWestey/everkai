...
export function enterpriseBreakdown(s,id){
 ...
 const qualityBonus=staffingStatus(id,b)?.bonus||0;
 const bonus=assignedOperation(s,definition)+qualityBonus;
 return {employees,operation,bonus,qualityBonus,total:(employees+operation)*(1+bonus)};
}
export function enterpriseRate(s){
 const rows=Object.entries(enterpriseState(s));if(!rows.length)return 0;
 const operation=rosterOperation(s);
 return rows.reduce((sum,[id,b])=>{const definition=BUSINESSES.find(d=>d.id===id);return sum+(definition?(employeeIncome(s,definition,b)+operation)*(1+assignedOperation(s,definition)+(staffingStatus(id,b)?.bonus||0)):0)},0);
}
export function businessAction(s,action,target,value){
 ...
 if(action==='openEnterprise'){
  ...
  return {state:{...s,enterprises:{...enterprises,[target]:{employees:0,fellows:[]}}},message:`Sandbox: ${definition.name} opened.`};
 }
