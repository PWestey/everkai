...
import {latencyBonus} from './latency.mjs';
import {familyStellaYield} from './family-stella.mjs';
...
export function businessBonus(s,id,b,definition){
 ...
 // LATENCY (2026-09-22). The original's `beautyquenching` neighbour: an ACCOUNT-WIDE sum over every
 // Family member's potentialCount, added to the same bracket as quality, Fathoms and the farm --
 // additive here, and multiplied by everything outside the bracket exactly once
 // (MainCityManager.lua:388-436). It applies to every building type, unlike Fathoms' per-type slots.
 const latency=latencyBonus(s);
 // FAMILY STELLA's `city | yield percent` halo (2026-09-22). Recorded in lib/family-stella-data.json's
 // 5th column since the import and deliberately unwired until now; docs/character-systems-gap.md 7.1
 // says it is the same decision as Latency, so it lands with it and under the same rule -- no cap.
 // Named apart from `family` (Fathoms) so a reader can tell the two Family strands from each other.
 const stella=familyStellaYield(s);
 return {quality,family,farm,latency,stella,total:assignedOperation(s,definition)+quality+family+farm+latency+stella};
}
export function enterpriseBreakdown(s,id){
 ...
 const {quality:qualityBonus,family:familyBonus,farm:farmBonus,latency:latencyBonus,stella:stellaYieldBonus,total:bonus}=businessBonus(s,id,b,definition);
 return {employees,operation,bonus,qualityBonus,familyBonus,farmBonus,latencyBonus,stellaYieldBonus,total:(employees+operation)*(1+bonus)};
}
