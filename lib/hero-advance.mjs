...
import {originRule} from './hero-stars.mjs';
...
/** Origin Boost (HeroLRSpSkill, step 4): the same subtree, the same shared shards, 10 a level to 600. */
export const originLevel=(s,id)=>heroAdvanceState(s).fellows[id]?.origin||0;
export {originRule};
...
export function heroAdvanceSpend(s){let n=0;for(const [id,x] of Object.entries(s?.heroAdvance?.fellows||{}))n+=(x.magic||0)*(magicRule(id)?.cost||0)+(x.pledge||0)*(pledgeRule(id)?.cost||0)+(x.origin||0)*(originRule(id)?.cost||0);return n?{[SPIRIT_SHARD_ITEM]:n}:{};}
export function validHeroAdvance(s){
 ...
 for(const [id,x] of Object.entries(t.fellows)){
  if(!(s.fellows[id]||REMOVED.has(id))||!x||typeof x!=='object'||Array.isArray(x)||Object.keys(x).some(k=>!['magic','pledge','origin'].includes(k)))return false;
  ...
  if(x.origin!==undefined&&(!originRule(id)||!int(x.origin,originRule(id).max)))return false;
 }
