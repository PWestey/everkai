...
// Coins are a FLAT one-shot grant per COMPLETED craft, not a per-second rate. The original's own
// description text says "+N/sec" and is wrong -- reward_simgame2.json grants the count once, via
const coins=(p,j,units)=>Math.floor(units*p.coins*(10+(j.masteryTier||0))/10);
...
export function validWorkshop(s){
 ...
 if(!object(w)||(w.restockDay!==undefined&&(typeof w.restockDay!=='string'||w.restockDay.length>10))||!['supplies','deposit','wallet'].every(k=>int(w[k]))||!object(w.crafted)||!object(w.salesXP))return false;
 ...
 return w.deposit+coins(p,j,j.count)-coins(p,j,j.unitsDone)<=1e9&&(w.salesXP[j.fellow]||0)+(j.count-j.unitsDone)*j.xpPerUnit<=1e9&&(w.crafted[p.id]||0)+j.count-j.unitsDone<=1e9;
}
export function settleWorkshop(s,now){
 ...
 if(!delta)return s;
 return {...s,workshop:{...w,deposit:w.deposit+coins(p,j,units)-coins(p,j,j.unitsDone),crafted:{...w.crafted,[p.id]:(w.crafted[p.id]||0)+completed},salesXP:{...w.salesXP,[j.fellow]:(w.salesXP[j.fellow]||0)+completed*j.xpPerUnit},job:units===j.count?null:{...j,secondsDone:seconds,unitsDone:units}}};
}
export function workshopAction(s,action,target,value){
 if(!['openWorkshop','restockWorkshop','startWorkshop','collectWorkshop','buyWorkshopPearl','upgradeWorkshopMastery'].includes(action))return null;
 ...
 // ECON-10: free and repeatable to 1e9. Now a daily habit reward, which also bounds the craft loop.
 if(action==='restockWorkshop'){const count=Math.max(0,Math.min(20,1e9-w.supplies));if(!count)return fail('Workshop Supplies are full.');const today=habitDay(s.lastAt);if(w.restockDay===today)return fail('Today’s supply delivery is already taken.');if(habitEarnings(s.habits,s.lastAt).dailies<1)return fail('Complete a daily habit for a supply delivery.');return result({...w,supplies:w.supplies+count,restockDay:today},`Daily habit reward: ${count} Workshop Supplies.`);}
 ...
 // finishWorkshop ("Finish now · Free") is retired (BUG-28): with a free restock it made crafting untimed and
 // unbounded -- 50 restock+craft+finish loops minted 1,800,000 coins in 0 ms of game time. Batches now take
 ...
 if(w.deposit+coins(p,{masteryTier},count)>1e9||(w.salesXP[fellow]||0)+count*xpPerUnit>1e9||(w.crafted[target]||0)+count>1e9)return fail('Make room for the complete batch rewards first.');
