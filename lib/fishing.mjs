...
import {FISH_ARTIFACTS,fishArtifactState,fishArtifactCounts,fishArtifactOwned,fishArtifactLevel,fishArtifactValue,fishArtifactUpgradeCost,fishArtifactPearl,pearls,pearlsFound,fishArtifactBonuses,fishArtifactExpMultiplier,fishArtifactBaitBonus,fishArtifactCrownBonus,fishArtifactYieldBonus,fishArtifactPowerEffects,fishArtifactTaxBp,castExtra,spotCatches,scriptedAt,fishValidArtifacts,PEARL,BLACK_PEARL,LEVELUP_PEARL_FROM,LEVELUP_PEARL_TO} from './fishing-artifacts.mjs';
export {FISH_ARTIFACTS,fishArtifactState,fishArtifactCounts,fishArtifactOwned,fishArtifactLevel,fishArtifactValue,fishArtifactUpgradeCost,fishArtifactPearl,pearls,pearlsFound,fishArtifactBonuses,fishArtifactTaxBp,PEARL,BLACK_PEARL};
/** All Building Earnings from the held artifacts, as a fraction -- lib/game.mjs buildingRate reads it
 *  exactly as it reads potionYield. Absent artifacts give 0, so no existing village rate moves. */
export const fishingArtifactYield=s=>fishArtifactYieldBonus(s?.fishing);
...
export function rollLength(n,id,crownBonus=0){
 const row=fishLengths(id);if(!row?.bands?.[0])return null;
 // A1403 "Legend Plate - 3" raises the Gold Crown band's own weight by its percent; with no artifact
 // crownBonus is 0 and the weights are the table's, so every stored band keeps its meaning.
 const weights=crownBonus?row.weights.map((w,i)=>i===LENGTH_BANDS-1?w*(1+crownBonus):w):row.weights,total=weights.reduce((a,b)=>a+b,0);if(!total)return null;
...
// FISHEXP IS PER SPECIES (measured after step 2): the owner's save keeps `normalExp` on each fish record, and his
// F1101 at level 7 is exactly its own 9 catches at 1 FishExp a catch. Pooled research points let the step-2
// pacing sim pour every duplicate into the few percent fish -- 12,900 bp of fishing percent on its top Fellow
// by day 30 against the owner's 1,250-3,250. So a level past 3 must ALSO be covered by that species' own
// researched duplicates (1 FishExp each), while still paying the pooled points as before.
/** FishExp the ladder past level 3 has consumed on one species. */
export const fishExpSpent=(id,n)=>{let t=0;for(let l=3;l<n;l++)t+=fishLevelCost(id,l);return t;};
/** That species' own researched duplicates: its FishExp. */
export const fishExp=(f,id)=>{let n=0;const idx=fishingIndex(f);for(const c of f.researched){if(idx.byId.get(c)?.fish===id)n++;}return n;};
/** Can this species step from `level` to level+1 on its own FishExp (always true below 3)? */
export const fishExpReady=(f,id,level)=>level<3||fishExp(f,id)-fishExpSpent(id,level)>=fishLevelCost(id,level);
...
export function validFishing(s){
 ...
 if(!Object.entries(f.skills).every(([id,n])=>n<=3||fishExpSpent(id,n)<=fishExp(f,id)))return false;
 if(!fishValidArtifacts(f,s.lastAt,draw.expPerCatch))return false;
 const spent=Object.entries(f.skills).reduce((sum,[id,n])=>sum+fishSkillSpent(id,n),0);return f.points===f.researched.length-spent;
}
// THE ARTIFACT SIDE OF A CAST (2026-09-22, lib/fishing-artifacts.mjs). The fish, the bait and the
// stored catch above are untouched: this only appends to `f.artifacts`, which a save written before
// this change does not have. Three things can happen, in the order the original resolves them:
//   1. FishSpot.FishUnspokenRules -- the scripted grant of the four `isCantGet` artifacts, at exactly
//      that spot's catch count (A1401 at the 15th Village River catch; A1402/03/04 at the 5th/16th/31st
//      of each Drakenberg spot). These are the only way to get them; the draw never can.
//   2. the extra roll on the original's full FishLevel denominator -- a rare-5 artifact, a Pearl or a
//      Black Pearl, at the original's own odds (see lib/fishing-artifacts.mjs note 1).
//   3. FishLevel.rewardNormal -- 1 Pearl for every fishing level gained from 63 to 500.
// A1401's fishing-EXP percent is banked as `bonusExp` at the moment it is earned rather than applied
// to the whole history, so a save's existing fishing level never moves under it.
function castArtifacts(before,next,ground,n,levelBefore,at){
 const a=fishArtifactState(before),found=[...a.found];
 const add=(kind,artifact)=>{
  found.push({id:`find:${found.length+1}`,kind,artifact:artifact||null,ground:ground||null,at,
   duplicate:kind==='artifact'&&found.some(x=>x.artifact===artifact)});};
 const spot=FISHING_GROUNDS[ground]?.spot||null;
 if(spot){
  for(const r of scriptedAt(spot,spotCatches(next.catches,FISHING_GROUNDS,spot)))add('artifact',r.id);
  const hit=castExtra(spot,levelBefore,castRoll(n,5),castRoll(n,6));
  if(hit)add(hit.kind,hit.artifact);
 }
 // The EXP this cast pays, and the extra the artifact adds on top of the flat FISH_EXP already counted.
 const bonusExp=(a.bonusExp||0)+Math.floor(draw.expPerCatch*(fishArtifactExpMultiplier(before)-1));
 let out={...next,artifacts:{...a,found,bonusExp}};
 const levelAfter=fishingLevel(fishingIndex(out).exp).level;
 for(let lv=levelBefore+1;lv<=levelAfter;lv++)if(lv>=LEVELUP_PEARL_FROM&&lv<=LEVELUP_PEARL_TO){found.push({id:`find:${found.length+1}`,kind:PEARL,artifact:null,ground:null,at,duplicate:false});}
 out={...out,artifacts:{...out.artifacts,found}};
 const names=found.slice(a.found.length).map(x=>x.kind==='artifact'?(FISH_ARTIFACTS.find(r=>r.id===x.artifact)?.name||x.artifact):x.kind===PEARL?'a Pearl':'a Black Pearl');
 return [out,names.length?` Also found ${names.join(' and ')}!`:''];
}
export function fishingAction(s,action,target,ground=null){
 if(!['baitRefill','castFish','displayFish','removeFish','researchFish','upgradeFish','claimCrownKohaku','crownKohaku','activateFishCombination','upgradeFishArtifact'].includes(action))return null;
 ...
 if(action==='baitRefill'){
   ...
   // A1402 "Legend Plate - 2" is "Baits Production SPD +5%" a level; Everkai pays the original's daily
   // supply through the habit refill, so it raises that day's allowance by the same fraction.
   const allowance=Math.floor(Math.min(BAIT_REFILL_MAX,dailies)*(1+fishArtifactBaitBonus(f)));
   const n=Math.max(0,Math.min(allowance,BAIT_STORAGE-f.bait));
 ...
 if(action==='castFish'){
  ...
  if(groundLevel(ground)===null)return fail('Choose a fishing ground.');const need=groundLevel(ground),have=fishingLevel(fishingIndex(f).exp).level;if(have<need)return fail(`${ground} opens at fishing level ${need}.`);const fish=drawFish(f,ground);if(!fish)return fail('Choose a fishing ground.');const n=f.catches.length+1,size=rollLength(n,fish.id,fishArtifactCrownBonus(f));const c={id:`catch:${n}`,fish:fish.id,name:fish.name,effect:{...fish.effect,rarities:[...fish.effect.rarities]},ground:ground||'All grounds',caughtAt:s.lastAt,policyVersion:size?4:2,duplicate:!!first(f,fish.id),...(size||{})};
  const back=c.duplicate?(f.catches.length%BAIT_DUPLICATE_RETURN===0?1:0):1;
  const next={...f,bait:Math.min(1000,f.bait-1+back),catches:[...f.catches,c]};
  const [after,extra]=castArtifacts(f,next,ground,n,have,s.lastAt);
   return done(after,`Caught ${c.name}${c.duplicate?' · ready for research':' · new collection entry'}${back?' · bait returned':''}!${extra}`);
 }
 ...
 if(action==='upgradeFishArtifact'){
  const r=FISH_ARTIFACTS.find(x=>x.id===target);if(!r)return fail('Choose an artifact.');
  if(!fishArtifactOwned(f,target))return fail('Fish this artifact up first.');
  const level=fishArtifactLevel(f,target),cost=fishArtifactUpgradeCost(target,level);
  if(cost===null)return fail('This artifact is at its level limit.');
  const kind=fishArtifactPearl(target);
  if(pearls(f,kind)<cost)return fail(`Fish up ${cost-pearls(f,kind)} more ${kind===PEARL?'Pearls':'Black Pearls'} first.`);
  const a=fishArtifactState(f);
  return done({...f,artifacts:{...a,levels:{...a.levels,[target]:level+1}}},`${r.name} level ${level+1} · ${r.describe?r.describe.replace(/\[[^\]]*\]/g,''):''}`.trim());
 }
 ...
 if(!f.displayed.includes(target))return fail('Display this fish before improving its skill.');if(level>=FISH_SKILL_MAX)return fail('This fish skill is at its level limit.');const cost=fishLevelCost(target,level);if(f.points<cost)return fail(`Research duplicates for ${cost} points first.`);if(!fishExpReady(f,target,level))return fail(`Past level 3 a fish levels on its own FishExp: research ${cost} more duplicates of ${c.name}.`);
