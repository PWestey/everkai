...
const empty={bait:0,catches:[],displayed:[],researched:[],skills:{},points:0};
...
export function fishingAction(s,action,target,ground=null){
 if(!['claimBait','castFish','displayFish','removeFish','researchFish','upgradeFish','claimCrownKohaku','crownKohaku','activateFishCombination'].includes(action))return null;
 ...
 if(action==='claimBait'){const n=Math.min(20,1000-f.bait);return n?done({...f,bait:f.bait+n},`Free bait +${n}.`):fail('Bait storage is full.');}
 if(action==='castFish'){
  ...
  return done({...f,bait:f.bait-1,catches:[...f.catches,c]},`Caught ${c.name}${c.duplicate?' · ready for research':' · new collection entry'}!`);
 }
