...
// The daily habit refill pays one bait per finished daily, up to 30. It was 10; once casts followed the
// original draw, 10 left the tank at 78M of a 255M roster by simulated day 21 (APK growth 301M/s).
// 30 measured 196M of 374M and 434M/s. The original's bait sources are not in the tables read, so the
// cap is a local pacing choice.
export const STARTING_BAIT=20,BAIT_REFILL_MAX=30,BAIT_DUPLICATE_RETURN=3;
...
export function fishingAction(s,action,target,ground=null){
 ...
 if(action==='castFish'){
  ...
  if(groundLevel(ground)===null)return fail('Choose a fishing ground.');const need=groundLevel(ground),have=fishingLevel(fishingIndex(f).exp).level;if(have<need)return fail(`${ground} opens at fishing level ${need}.`);const fish=drawFish(f,ground);if(!fish)return fail('Choose a fishing ground.');const c={id:`catch:${f.catches.length+1}`,fish:fish.id,name:fish.name,effect:{...fish.effect,rarities:[...fish.effect.rarities]},ground:ground||'All grounds',caughtAt:s.lastAt,policyVersion:2,duplicate:!!first(f,fish.id)};
