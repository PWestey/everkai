...
import {FAMILY,fellowById} from './catalog.mjs';
 ...
 *  The family branch still reads FAMILY directly, which is correct while every Family record is an
 *  original. docs/crossover-family-split.md plans ~30 crossover FAMILY members; that slice has no
 *  additions layer yet, and when it lands this branch needs the same additions-aware lookup. */
export const stagePerson=stage=>stageKind(stage)==='fellows'?fellowById(stage?.member)||null:FAMILY.find(p=>p.id===stage?.member)||null;
