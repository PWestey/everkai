...
import skills from './talent-skill-data.json' with {type:'json'};
/** RAISED 2026-09-18 to the ALL-SOURCES ceiling, 107,198 (docs/power-sources-import-spec.md 1.3):
 *  scripts/import-talent-skills.py measures every talent skill a hero can ever own -- Hero.json's, every
 *  Rarity Advance stage's, the Stella-unlocked, star and pledge skills, plus the fixed intimacy skill -- at
 *  maxUpgradeLevel + that hero's whole talentLvLimit raise; hero_253/254 top it. The 31,122 above bounded
 *  Hero.json's skills only. A WIDENING, so every save that decodes today still decodes (rule 12). The new
 *  talent skills themselves never write `f.aptitude` (lib/talent-skills.mjs derives them), so this bound
 *  still governs exactly the sources it always did; it simply no longer sits below what the original's
 *  own tables allow one hero to reach. */
export const APTITUDE_CAP=Math.max(data.cap,skills.ceiling.cap);
