...
/** The original's own order, corrected 2026-09-18. Everkai multiplied the owner's flat by the typed
 *  percent -- floor((power + flat) * (1 + percent/100)) -- and its own UI said so while admitting
 *  "original stacking is unverified". It is verified now: the client computes
 *    base * (1 + percent/10000) * ... + extradd
 *  (private-server/readable/PropManager.lua:116), i.e. the Spirit's flat `extradd` is added AFTER every
 *  multiplier, not inside one. MEASURED before/after on the shipped ceiling fixture: 4,655,637 ->
 *  4,519,467 flag-off with today's three reachable tracks (-2.9%), which is the only way a player
 *  mid-save sees Power fall -- and only a Fellow who BOTH owns a levelled track AND is of a type with an
 *  activated percent is affected at all. It also makes importing 111 real ladders affordable: the same
 *  import is 7.13x of the original's live-save reference under the old order and 4.47x under this one.
 *  tests/stella.test.mjs pins both numbers. */
export function applyStella(s,id,power){const b=stellaBonus(s,id);return Math.floor(power*(1+b.percent/100))+b.flat;}
...
const priced=(p,r)=>!p.reprice||(r.level===0
 ?r.flat===(p.activation?.flat||0)&&r.percent===(p.activation?.percent||0)
 :r.flat===p.levels[r.level-1].flat&&r.percent===p.levels[r.level-1].percent);
