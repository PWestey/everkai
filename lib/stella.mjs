...
import {SPIRIT_PROFILES,spiritProfile,spiritShard,ownsSpiritShard,SHIPPED_SPIRIT,SPIRIT_SHARD_ITEM,LEGACY_OWNER_ITEMS} from './hero-spirit.mjs';
import {ADDITION_FELLOWS} from './everkai-additions.mjs';
import {heroAdvanceSpend} from './hero-advance.mjs';
import {familyStellaSpend} from './family-stella.mjs';
...
export {CROSSOVER_STELLA,crossoverStella,spiritShard,SPIRIT_SHARD_ITEM,LEGACY_OWNER_ITEMS};
/** Every item a Stella ledger may name: each ladder's spend item (the village pool, the crossover pool)
 *  plus the four retired private fragment items, which are no longer minted but which saves still hold.
 *  This -- not the set of ladders' CURRENT spend items -- is what validStella reconciles, so a save that
 *  holds a private item decodes exactly as it did when that item was still minted (CLAUDE.md rule 12). */
export const STELLA_ITEMS=Object.freeze([...new Set([...ALL_STELLA_PROFILES.map(p=>p.itemId),...LEGACY_OWNER_ITEMS])]);
const stellaItem=id=>STELLA_ITEMS.includes(id);
...
export function settleStella(s,from,to){
 ...
 // NO PRIVATE MINT, 2026-09-19. Every ladder's `itemId` is now a POOL (the village shard or the crossover
 // shard); the four shipped ladders' private fragment items survive only as `ownItemId`, which this loop
 // never reads. So the faucet is at most two streams -- village and crossover -- however the roster looks.
...
export function stellaBonus(s,id){const own=stellaEntry(s,id);
 // The crossover track's Power is read off the CURRENT ladder by the level reached, not off the receipt: a
 // receipt written before CROSSOVER_FLAT_SCALE (lib/crossover-stella.mjs) records Angie's unscaled flat, and the
 // level it bought is what the player paid for. Percent stays zero on both ladders.
 if(crossoverStella(id)){const row=own&&own.level>0?CROSSOVER_STELLA.levels[own.level-1]:null;return {flat:row?.flat||0,percent:row?.percent||0,selfPercent:0,typedPercent:row?.percent||0};}
 ...
 *  crossover ladder IS repriced: every row must carry its own ladder's flat and percent, and the free
 *  activation must carry zeros. CORRECTED 2026-09-19: this said the track "has never shipped" -- it has,
 *  main@45828d3 shows crossover characters by default, so live saves hold its rows at Angie's unscaled flat.
 *  When the flat column was scaled for crossover parity those rows stayed legal through `legacyLevels`
 *  (tests/crossover-stella.test.mjs pins a save the live build wrote). */
...
// A repriced ladder that was re-scaled after it shipped (the crossover track, 2026-09-19) keeps accepting the
// rows its previous ladder wrote: `legacyLevels`. Exact values of one of the two ladders, nothing in between.
const priced=(p,r)=>!p.reprice||pricedRow(r,r.level===0?p.activation:p.levels[r.level-1])||(r.level>0&&!!p.legacyLevels&&pricedRow(r,p.legacyLevels[r.level-1]));
...
export function validStella(s){if(s?.stella===undefined)return true;const t=s.stella,int=(n,max=1e9)=>Number.isSafeInteger(n)&&n>=0&&n<=max;if(!t||t.policyVersion!==1||!int(t.seq)||!t.stock||Array.isArray(t.stock)||!Object.entries(t.stock).every(([id,n])=>stellaItem(id)&&int(n,1e6))||!Array.isArray(t.grants)||t.grants.length>10000||!Array.isArray(t.history)||t.history.length>STELLA_HISTORY_MAX)return false;
 if(t.since!==undefined&&(!int(t.since,Number.MAX_SAFE_INTEGER)||t.since>s.lastAt))return false;
 if(t.idle!==undefined&&(!t.idle||Array.isArray(t.idle)||typeof t.idle!=='object'||!Object.entries(t.idle).every(([id,n])=>stellaItem(id)&&int(n,1e6))))return false;
 // `converted` (2026-09-19): retired private fragments moved 1:1 into the village pool by stellaConvert.
 // Absent on every save written before it, which therefore reconciles exactly as before.
 if(t.converted!==undefined&&(!t.converted||Array.isArray(t.converted)||typeof t.converted!=='object'||!Object.entries(t.converted).every(([id,n])=>LEGACY_OWNER_ITEMS.includes(id)&&int(n,1e9)&&n>0)))return false;
 const balances={},ids=new Set(),levels={};for(const r of t.grants){if(!r||!int(r.id)||r.id>t.seq||ids.has(r.id)||!stellaItem(r.itemId)||!int(r.count,1e6)||!r.count||!int(r.at,Number.MAX_SAFE_INTEGER)||r.at>s.lastAt)return false;ids.add(r.id);balances[r.itemId]=(balances[r.itemId]||0)+r.count;}
 ...
 // Family Stella ranks (lib/family-stella.mjs) spend the same pool, derived the same way.
 const spent=heroAdvanceSpend(s),family=familyStellaSpend(s);for(const [k,n] of Object.entries(family))spent[k]=(spent[k]||0)+n;
 for(const [k,n] of Object.entries(t.converted||{})){balances[k]=(balances[k]||0)-n;balances[SPIRIT_SHARD_ITEM]=(balances[SPIRIT_SHARD_ITEM]||0)+n;}
 return STELLA_ITEMS.every(k=>(balances[k]||0)+(t.idle?.[k]||0)-(spent[k]||0)===(t.stock[k]||0));
}
/** Which item pays each row. A shipped ladder spends its retired private fragments FIRST -- they buy
 *  nothing else unless converted -- and the shared pool after; a row is paid wholly from one item, so its
 *  receipt names one `itemId` exactly as every receipt always has. A private remainder smaller than the
 *  next row is left for stellaConvert rather than split across two items. `paid` is the spend per item. */
export function stellaPlan(s,id,count=/** @type {number|string} */ (1)){const p=stellaRule(id),entry=stellaEntry(s,id),t=stellaState(s);if(!p||!entry||![1,5,'max'].includes(count))return {rows:[],cost:0,paid:{}};
 const left={[p.itemId]:t.stock[p.itemId]||0};if(p.ownItemId)left[p.ownItemId]=t.stock[p.ownItemId]||0;
 const rows=[],paid={};let cost=0;
 for(const row of p.levels.slice(entry.level,entry.level+(count==='max'?40:count))){
  const item=p.ownItemId&&left[p.ownItemId]>=row.cost?p.ownItemId:left[p.itemId]>=row.cost?p.itemId:null;
  if(!item||t.seq+rows.length+1>1e9)break;
  left[item]-=row.cost;paid[item]=(paid[item]||0)+row.cost;rows.push({...row,itemId:item});cost+=row.cost;}
 return {rows,cost,paid};}
/** One-way, 1:1: a retired private fragment item (LEGACY_OWNER_ITEMS) into the village shard pool. What
 *  fits under the pool's 1e6 cap moves; the rest stays. Recorded in `converted` so the ledger still adds up. */
function stellaConvert(s,item,value){const old=stellaState(s),fail=error=>({state:s,error});
 if(!LEGACY_OWNER_ITEMS.includes(item))return fail('Only a retired private fragment can be converted.');
 if(!s.stella||value?.seq!==old.seq||old.seq>=1e9)return fail('Stella changed. Use current controls.');
 const held=old.stock[item]||0,pool=old.stock[SPIRIT_SHARD_ITEM]||0,n=Math.min(held,1e6-pool);
 if(n<=0)return fail(held?'The village shard pool is full.':'No fragments of this kind are left to convert.');
 const converted={...(old.converted||{})};converted[item]=(converted[item]||0)+n;
 return {state:{...s,stella:{...old,seq:old.seq+1,stock:{...old.stock,[item]:held-n,[SPIRIT_SHARD_ITEM]:pool+n},converted}},
  message:`${n.toLocaleString()} private fragments converted into ${n.toLocaleString()} village shards${n<held?` (${(held-n).toLocaleString()} left: the pool is full)`:''}.`};}
export function stellaAction(s,action,target,value){if(action==='stellaConvert')return stellaConvert(s,target,value);if(!['stellaActivate','stellaUpgrade'].includes(action))return null;const old=stellaState(s),p=stellaRule(target),fail=error=>({state:s,error});if(!p||!s.fellows[target]||(!shared(p)&&fellowById(target)?.type!==p.type))return fail('Choose an owned Fellow with a supported Stella curve.');if(value?.seq!==old.seq||old.seq>=1e9)return fail('Stella changed. Use current controls.');const t={...old,seq:old.seq+1,stock:{...old.stock}},balance=t.stock[p.itemId]||0,entry=stellaEntry(s,target);
