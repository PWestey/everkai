...
export function settleStella(s,from,to){
 const old=s.stella;if(!old||!Number.isFinite(from)||!Number.isFinite(to)||to<=from)return s;
...
export function validStella(s){if(s?.stella===undefined)return true;const t=s.stella,int=(n,max=1e9)=>Number.isSafeInteger(n)&&n>=0&&n<=max;if(!t||t.policyVersion!==1||!int(t.seq)||!t.stock||Array.isArray(t.stock)||!Object.entries(t.stock).every(([id,n])=>STELLA_PROFILES.some(p=>p.itemId===id)&&int(n,1e6))||!Array.isArray(t.grants)||t.grants.length>10000||!Array.isArray(t.history)||t.history.length>STELLA_PROFILES.reduce((n,p)=>n+p.levels.length+1,0))return false;
 ...
 if(t.idle!==undefined&&(!t.idle||Array.isArray(t.idle)||typeof t.idle!=='object'||!Object.entries(t.idle).every(([id,n])=>STELLA_PROFILES.some(p=>p.itemId===id)&&int(n,1e6))))return false;
 ...
 return STELLA_PROFILES.every(p=>(balances[p.itemId]||0)+(t.idle?.[p.itemId]||0)===(t.stock[p.itemId]||0));
}
...
export function stellaAction(s,action,target,value){if(!['stellaActivate','stellaUpgrade'].includes(action))return null;const old=stellaState(s),p=stellaRule(target),fail=error=>({state:s,error});if(!p||!s.fellows[target]||fellowById(target)?.type!==p.type)return fail('Choose an owned Fellow with a supported Stella curve.');if(value?.seq!==old.seq||old.seq>=1e9)return fail('Stella changed. Use current controls.');const t={...old,seq:old.seq+1,stock:{...old.stock}},balance=t.stock[p.itemId]||0,entry=stellaEntry(s,target);
