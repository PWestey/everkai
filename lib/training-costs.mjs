...
// The imported table stops at level 59. ONE shared fall-back, used by both the charge
// (adventure.mjs xpCost) and the receipt recomputation below. Written separately, they disagreed:
// the game billed originalCost(60) while the validator held that no price existed past 59, so
// every save that trained a Fellow past level 60 was rejected by valid().
export const trainingCost=level=>originalTrainingCost(level)??originalCost(level)??null;
