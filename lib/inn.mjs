...
export function validInn(s){
 ...
 if(!Object.entries(i.stations).every(([id,l])=>stations.has(id)&&int(l,20)&&l>=1&&i.served>=stations.get(id).guests))return false;
 ...
 if(q&&q.gains!==undefined&&(!object(q.gains)||!int(q.gains.finesse,240)||q.gains.finesse<1||!int(q.gains.popularity,140)))return false;
...
export function innAction(s,action,target,value){
 ...
 if(action==='buildInnStation'||action==='upgradeInnStation'){
  ...
  if(!level||level>=20)return fail('Build the station or choose one below level 20.');if(i.blueprints<level)return fail('Serve guests for more blueprints.');
