...
/** enemySkills (combat version 11, 2026-09-16): enemy units carrying `pet` also cast that familiar's active
 *  skill at full Rage. Versions 10 and earlier never pass it, so their replays are byte-identical. */
export function criticalBattle(initial,floor,skillFor,{enemySkills=false}={}){
 ...
 for(let round=1;round<=15;round++){
  ...
  for(const u of order){
   ...
   const skill=u.rage===100,rule=skill&&u.side===0?skillFor(u.id):skill&&enemySkills&&u.pet?skillFor(u.pet):null;
   if(rule){
