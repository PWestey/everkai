...
export function familiarBonus(s,fellow){
 ...
 if(!pet)return bonus;
 // A bond pays a ninth of the inherent bonus per stage past the first: nothing at stage 1 (levels 1-49),
 // the full bonus at stage 10 (level 450+). Every step needs the class-up items the tower pays, so the
 // bonus is earned through training rather than granted on the bond (ECON-29). The per-stage share is a
 // local rule; the original scales its hero bonus by star (PetStar.ExternalAdd1), not yet imported.
 const share=(Math.min(10,Math.floor((s.familiars?.[pet]?.level||1)/50)+1)-1)/9;
 for(const [field,value] of Object.entries(data.inherent[pet]||{}))bonus[field]=field==='flat'?Math.floor(value*share):Math.round(value*share*100)/100;
