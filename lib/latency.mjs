...
import {fathomsApply} from './fathoms.mjs';
// The balance lives in lib/luck-stones.mjs because ADVANCED FATHOMS spend the same stone -- in the
// original the two systems compete for exactly one currency, so Everkai has to price them against
// one number. That module imports neither this one nor fathoms, which is what keeps the three of
// them acyclic.
import {luckStones,luckStonesEarned,luckStonesSpent,STONES_PER_ACTION,LUCK_STONE as STONE_ITEM} from './luck-stones.mjs';
 ...
 *  recommends: the habit counter Fathoms already gates on. That counter is monotonic, unbuyable and
 *  never reset, so the balance needs no storage at all -- earned minus spent, both derived, and it is
 *  shared with Advanced Fathoms in lib/luck-stones.mjs exactly as the original shares it.
 *
...
export const LUCK_STONE=STONE_ITEM;
...
/** The faucet rate. LOCAL, and the only invented number in this system; it lives in
 *  lib/luck-stones.mjs with the rest of the ledger and is re-exported here so the panel can name it.
 *
...
export {STONES_PER_ACTION,luckStones,luckStonesEarned,luckStonesSpent};
