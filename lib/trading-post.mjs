...
import {NOTES_PER_WIN} from './operations.mjs';
...
// STUDY NOTES ride on exactly the same rule (2026-09-22). Item:source for
// Item_HeroManagerment_Building names the "Trading Post Shop", and the shop's own stock table is not in
// the config dump either -- positive control: the same sweep reads ResourceShop, guildShop, highwayShop
// and RandomShop rows, and none of them prices the item. So the rate is Everkai's, it is declared as
// NOTES_PER_WIN in lib/operations.mjs beside the ladder it pays for, it is written onto each receipt,
// and a receipt from before this change is read at the constant. That only ever adds notes.
...
function validRun(r,s){return r&&r.policyVersion===1&&int(r.id)&&r.simulated===true&&r.opponent&&TRADE_OPPONENTS.some(x=>x.id===r.opponent.id)&&int(r.opponent.power,1e15)&&typeof r.opponent.name==='string'&&r.opponent.name.length<80&&Array.isArray(r.team)&&r.team.length>0&&r.team.length<=6&&new Set(r.team.map(p=>p?.id)).size===r.team.length
 ...
 &&r.team.every(p=>p&&(s.fellows?.[p.id]||REMOVED.has(p.id))&&typeof p.name==='string'&&p.name.length<80&&int(p.power,1e15)&&p.paidEnergy===1&&p.won===(p.power>=r.opponent.power))&&r.wins===r.team.filter(p=>p.won).length&&int(r.coinPerWin,1e6)&&int(r.influencePerWin,1e6)&&(r.voucherPerWin===undefined||int(r.voucherPerWin,1e6))&&(r.notePerWin===undefined||int(r.notePerWin,1e6))&&r.coins===r.wins*r.coinPerWin&&r.influence===r.wins*r.influencePerWin&&Number.isSafeInteger(r.startedAt)&&r.startedAt>=0&&r.startedAt<=s.lastAt&&Number.isSafeInteger(r.readyAt)&&r.readyAt>r.startedAt}
