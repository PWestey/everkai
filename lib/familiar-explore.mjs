...
export function validFamiliarExplore(s){
 ...
 // already gives away.
 if(e.exchange!==undefined){
  const x=e.exchange;
  if(!record(x)||Object.keys(x).length!==2||typeof x.day!=='string'||x.day.length>10||!int(x.count,exchange.limit))return false;
 }
...
export function exploreAction(s,action,target,value){
 if(!['familiarStarter','exploreArea','exploreStep','exploreResolve','exploreCatch','exploreSoothe','exploreExchange'].includes(action))return null;
