...
import {familiarSupplies,towerFloorOf} from './familiar-supplies.mjs';
...
export const areaUnlocked=(s,id)=>{const a=exploreArea(id);return !!a&&towerFloorOf(s)>=a.unlock;};
...
export function validFamiliarExplore(s){
 ...
 if(!record(e.pieces)||!Object.entries(e.pieces).every(([id,n])=>Object.hasOwn(owned,id)&&int(n,ITEM_MAX)&&n>0))return false;
