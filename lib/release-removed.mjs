...
// Releases characters Everkai no longer ships from an old save. When a character leaves the catalog the
// validators have no entry for them, so a save that owned one fails validation and will not load at all.
// The owner chose release: the character leaves, everything else stays. Each rule below removes a
// reference the validators tie to an owned Fellow or Family member; anything a player spent on the
// character stays spent. Inert when no removed id appears anywhere in the save.
//
// REMOVED is EMPTY as shipped (the Demon Slayer crossover cast was removed 2026-09-11 and restored
// 2026-09-15), so this pipeline is currently a no-op on every save. It stays because the next cast change
// needs it, and because a mechanism only tested through whatever happens to be removed today is untested
// the day it matters: both functions take the set as an argument so tests drive them directly.
...
export function releaseRemoved(s,removed=REMOVED){
 if(!obj(s)||!mentionsRemoved(s,removed))return s;
