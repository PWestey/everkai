...
export function validAptitudeLedger(s){
 return Object.values(s.fellows).every(f=>{
  ...
  for(const [key,e] of list){
   if(!aptitudeLedgerKey(key)||!e||typeof e!=='object'||Object.keys(e).length!==2||!Number.isSafeInteger(e.paid)||e.paid<1||e.paid>1e12||!Number.isInteger(e.gain)||e.gain<1||e.gain>APTITUDE_CAP)return false;
   gain+=e.gain;
  }
