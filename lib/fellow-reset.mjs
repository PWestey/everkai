...
import {originalProgression,qualityRule,paidQuality,localTier} from './original-progression.mjs';
...
import {TALENT_SKILLS,SKILL_PEARL,heroTalentSkills,talentSkillUnlocked} from './talent-skills.mjs';
import {heroAdvanceState,magicRule,pledgeRule,originRule} from './hero-advance.mjs';
...
export function refundPlan(s,id,costs=REFUND_COSTS){
 ...
 // Quality breakthroughs, newest first, only while the remaining level still fits the lower cap. The tier
 // stored back is the one the REMAINING receipts chain to -- not the removed receipt's `from`, which since
 // 2026-09-22 may sit above the chain where limit breaks granted the tiers in between.
 let needTier=1;
 if(t.quality){
  const p=s.originalProgression,receipts=[...p.receipts],stock={...p.stock};let changed=false;
  const chain=()=>{let q=1;for(const r of receipts)if(r.id===id)q=r.to;return q};
  while(chain()>1){const i=receipts.findLastIndex(r=>r.id===id);if(i<0||fellow.level>qualityRule(receipts[i].from).cap)break;for(const c of receipts[i].cost){stock[c.id]+=c.count;add('breach:'+c.id,c.count);}receipts.splice(i,1);changed=true;}
  const q=chain();
  ...
  // A KEPT receipt that starts above the chain was only legal because limit breaks granted the tiers it
  // skipped, so those breaks are still doing work and the loop below must not hand their tokens back.
  // DEFENCE, NOT A LIVE FIX, and measured as such: deleting this line fails no test, because the level
  // refund above always resets the Fellow to his baseline first, and the baseline fit `localTier`'s cap at
  // activation -- so every receipt is removable and nothing is ever kept that needs a skip. It is here for
  // the case that is not true of: levels gained after activation WITHOUT a training receipt (the opening's
  // item-priced table is the one that already does this). Returning the tokens then costs the player the
  // entire refund, because fellowResetAction refuses a plan `valid()` rejects.
  let cur=1;for(const r of receipts)if(r.id===id){if(r.from>cur)needTier=Math.max(needTier,r.from);cur=r.to;}
 }

 // Limit breaks, newest first, while the level still fits without them. ONE rule for both modes since
 // 2026-09-22: APK growth's cap now also reads `breaks` (lib/original-progression.mjs sourceQuality), so the
 // old `originalProgression(s)||` short-circuit would have returned every limit token while the Fellow kept
 // the cap those tokens bought -- a token faucet, and refundFellow's promise is that it never gives back
 // anything that is still doing work. The cap is read against `next`, which already holds the receipts this
 // refund removed: reading it against `s` priced the breaks at a quality the Fellow no longer has and
 // returned tokens his kept level still needed, which `valid()` then refused -- losing the whole refund.
 while(fellow.breaks>0&&localTier({...fellow,breaks:fellow.breaks-1})>=needTier&&fellow.level<=fellowCap({...fellow,breaks:fellow.breaks-1},next,id)){add('item:local_limit_token',costs.breaks(fellow.breaks-1));fellow.breaks--;}
 ...
 if(t.stella&&stellaEntry(s,id)){
  // Each receipt is refunded in the item IT names, not the ladder's current spend item: since 2026-09-19 a
  // shipped ladder spends the village pool, but its older receipts were paid in its private fragment item,
  // and validStella reconciles per item -- returning those to the pool would break the ledger identity.
  const old=stellaState(s),paidBy={};
  for(const r of old.history)if(r.owner===id&&r.level>0)paidBy[r.itemId]=(paidBy[r.itemId]||0)+r.paid;
  if(Object.keys(paidBy).length){const stock={...old.stock};for(const [k,n] of Object.entries(paidBy)){add('stella:'+k,n);stock[k]=(stock[k]||0)+n;}
   next.stella={...old,seq:Math.min(1e9,old.seq+1),stock,history:old.history.filter(r=>!(r.owner===id&&r.level>0))};}
 }

 // Talent skills (lib/talent-skills.mjs): every stored level above the free level 1 cost a constant count
 // of one item -- Skill Pearls to the bag, Country books to that type's Insight balance.
 if(fellow.talentSkills){
  for(const [skill,n] of Object.entries(fellow.talentSkills)){const r=TALENT_SKILLS[skill];if(!r)continue;const paid=(n-1)*r.p;
   if(r.c===SKILL_PEARL)add('item:'+r.c,paid);
   else{add('insight:'+r.c,paid);const i=next.insight||insightState(s);next.insight={...i,balances:{...i.balances,[r.c]:(i.balances[r.c]||0)+paid}};}}
  delete fellow.talentSkills;
 }
 // Rarity Advance and Pledge (lib/hero-advance.mjs): shards back to the shared pool. The spend is derived from
 // the stored levels, so removing the levels and returning the shards keeps validStella's identity exact.
 const adv=heroAdvanceState(s).fellows[id];
 if(adv){const paid=(adv.magic||0)*(magicRule(id)?.cost||0)+(adv.pledge||0)*(pledgeRule(id)?.cost||0)+(adv.origin||0)*(originRule(id)?.cost||0);
  if(paid){add('stella:'+SPIRIT_SHARD_ITEM,paid);const st=next.stella||stellaState(s);next.stella={...st,stock:{...st.stock,[SPIRIT_SHARD_ITEM]:(st.stock[SPIRIT_SHARD_ITEM]||0)+paid}};}
  const rest={...heroAdvanceState(s).fellows};delete rest[id];next.heroAdvance={policyVersion:1,fellows:rest};
 }

 // Quenching (lib/quench.mjs) is paid in gold, and Refund never returns gold: it stays on the Fellow.
 if(fellow.quench&&Object.values(fellow.quench).some(a=>a.some(r=>r>0)))kept.push('artifact quenching (paid in gold, which Refund never returns)');
...
function candidates(s,id){
 ...
 // Talent skills and Rarity Advance / Pledge: Refund all returns both exactly (refundPlan above).
 for(const {skill} of heroTalentSkills(id))if(talentSkillUnlocked(s,id,skill))list.push(['trainTalentSkill',{skill,amount:5}]);
 if(magicRule(id))list.push(['rarityAdvance',10]);
 if(pledgeRule(id))list.push(['pledgeAdvance',10]);
 if(originRule(id))list.push(['originBoost',10]);
 return list;
}
