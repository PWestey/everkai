...
export function refundPlan(s,id,costs=REFUND_COSTS){
 ...
 // 2026-09-22: APK growth's cap now also reads `breaks` (lib/original-progression.mjs sourceTier), so the
 ...
 // anything that is still doing work.
 while(fellow.breaks>0&&fellow.level<=fellowCap({...fellow,breaks:fellow.breaks-1},s,id)){add('item:local_limit_token',costs.breaks(fellow.breaks-1));fellow.breaks--;}
