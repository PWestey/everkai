...
export const IDLE_EXP_WINDOW_MS=12*60*60*1000;
/** Local rule. Stage clears were 94% of Fellow EXP (measured: 4.77M of 5.09M over five simulated days),
 *  so EXP stopped when the stages did. The journey now also pays, every whole minute, the Fellow EXP
 *  (BattleNormal item1) of the last normal stage cleared, for up to the 12h away window. The original's
 *  idle EXP rate is not in the tables read. Measured with 10 breakthrough materials a day: APK growth
 *  day 7/14/21 234M/426M/747M/s; default growth 21M/28M/38M -> 28M/45M/68M/s. */
export function idleExpRate(s){const c=s.opening?.cleared||0;for(let i=Math.min(c,OPENING_STAGES.length)-1;i>=0;i--)if(!OPENING_STAGES[i].boss)return OPENING_STAGES[i].item1||0;return 0;}
/** Pays the clock-minute boundaries crossed between `from` (the save's previous lastAt) and `to`, so no
 *  anchor is stored and frequent settles lose nothing; an absence pays at most 720 minutes. */
export function settleOpeningExp(s,from,to){
 if(!s.opening||!(to>from))return s;
 const minutes=Math.min(IDLE_EXP_WINDOW_MS/60000,Math.floor(to/60000)-Math.floor(from/60000)),rate=idleExpRate(s);
 return minutes>0&&rate>0?{...s,fellowXP:Math.min(MAX_FELLOW_XP,s.fellowXP+minutes*rate)}:s;
}
