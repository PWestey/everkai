...
export function openingAction(s,action,target,value,income){
 ...
 case 'openingReforge':{const f=n.fellows[target];if(!f?.gear||f.aptitude>=1000)throw Error('Choose an equipped Fellow below the Aptitude cap.');changeItem(n,'Item_Quenching_Equipment_1',-1);f.aptitude++;n.fellows[target]=recordAptitude(f,'item:Item_Quenching_Equipment_1',1,1);o.reforges++;break;}
 ...
 else if(row.useEffect==='addAtk'){if(!n.fellows[value])throw Error('Choose a Fellow.');const f=n.fellows[value];const gain=row.useParam.count/100; if(f.aptitude+gain>1000)throw Error('Aptitude cap reached.');f.aptitude+=gain;n.fellows[value]=recordAptitude(f,'item:'+target,1,gain);}
 ...
 else if(target==='Item_Box_Talent_1'||target.startsWith('Item_Hero_Talent_Country_')){const f=n.fellows[value];if(!f||f.aptitude>=1000)throw Error('Choose a Fellow below the Aptitude cap.');f.aptitude++;n.fellows[value]=recordAptitude(f,'item:'+target,1,1);}
