...
// Rank costs are the original's own Level.json rows 1-5, imported verbatim -- and they assume the
// original's stage ladder, which is ~240,000 normal rows. Everkai ships 126 encounters, so the whole
// Fame supply is 294 (240 from 120 stages + 48 from 6 bosses + 6 from tasks/events, all verified as
// faithful imports) against a cumulative 305. Rank 5 was unreachable by 11 Fame no matter how well
// the player played. Rank 5's cost is therefore the one LOCAL balance number in the ladder: 108
// makes it reachable from the stage ladder and bosses alone (288), leaving the 6 task/event Fame as
// slack rather than demanding a literally perfect sweep. Ranks 1-4 remain the original's values.
export const RANK_EXP_OVERRIDE={5:108};
/** Fame charged to promote OUT of `rank`. The override is local balance; everything else is source. */
export const rankCost=rank=>RANK_EXP_OVERRIDE[rank]??data.ranks[rank-1]?.expNeed;
...
function changeItem(s,id,amount){
 ...
 if(id==='1'){owner=s;key='fellowXP';max=MAX_FELLOW_XP}else if(id==='3'){owner=s;key='gold';max=1e12}else if(id==='4'){owner=s;key='crystals';max=1e9}else if(id==='5'){owner=o;key='fame';max=1e9}else {owner=Object.hasOwn(s.inventory,id)?s.inventory:o.locker;key=id;max=1e6;}
...
export function openingAction(s,action,target,value,income){
 ...
 case 'openingPromote':{const r=data.ranks[o.rank-1];if(o.rank>=5)throw Error('The opening rank journey is complete.');if(openingProsperity(s,income)<r.prosperityNeed)throw Error('Build more prosperity first.');changeItem(n,'5',-rankCost(o.rank));o.rank++;break;}
