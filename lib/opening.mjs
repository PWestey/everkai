...
import {recordAptitude} from './aptitude-ledger.mjs';
...
import {decodeCampaign,CAMPAIGN,OPENING_STAGES,OPENING_COUNT,LAST_CHAPTER,openingPending,openingQuote,openingBossReady} from './stage-ladder.mjs';
import {ladderPower,newFellow,fellowCap,adventureAction,FRESH_FOUNTAIN} from './adventure.mjs';
...
// Chapters 1-6 are the opening journey; chapters 7-3,000 continue the original's own stage ladder
// (scripts/import-campaign-chapters.py). Their roadside events are deferred -- see that script.
// The decode, the row order and the original's two battle rules now live in lib/stage-ladder.mjs, so
// the village campaign reads the SAME 63,000 rows instead of importing a second copy of the table.
// Re-exported here unchanged: every existing importer of these names keeps working.
export {decodeCampaign,CAMPAIGN,OPENING_STAGES,OPENING_COUNT,LAST_CHAPTER,openingPending,openingQuote,openingBossReady};
...
/** All 22 rank-up encounters exactly as the original's CityAssignEvent table has them. This is the
 *  allow-list `o.city` is validated against and the table its ranks are read from, so it must stay
 *  whole: 14 of the 22 name a Fellow the owner deleted on 2026-09-17, and a save that had already met
 *  one still carries that encounter id. Drop the row and `validOpening` either refuses the save or
 *  throws looking up its rank. */
export const RANK_ENCOUNTER_ROWS=ladder.encounters;
/** The rank-up encounters the game can actually offer: encounter id, the rank that unlocks it, and the
 *  Fellow it brings. Filtered to Fellows Everkai still ships, so the Rank panel does not advertise a
 *  "Meet undefined" button for a Fellow who no longer exists and openingRecruit cannot be asked for one.
 *  The ladder itself is untouched and still works: promotion is paid in Fame and gated on earnings
 *  (rankCost/rankEarnings), and an encounter has never been a promotion requirement. Ranks 3, 6, 7, 8,
 *  9, 13, 15, 16, 18, 19, 22, 24, 25 and 27 simply have no Fellow waiting any more -- 8 of the 22
 *  remain, and rank 28 (Cimitir) still closes the ladder. */
export const RANK_ENCOUNTERS=RANK_ENCOUNTER_ROWS.filter(e=>FELLOWS.some(f=>f.id===e.fellow));
...
export function validOpening(s){
 if(s?.opening===undefined)return true;const o=s.opening;if(!obj(o)||o.version!==1||!int(o.cleared,OPENING_COUNT)||!int(o.rank,MAX_RANK)||o.rank<1||!int(o.claimed,136)||!['fame','collections','education','fathoms','reforges','upgrades'].every(k=>int(o[k])))return false;
 ...
 if(!unique(o.city,RANK_ENCOUNTER_ROWS.map(e=>e.id))||o.city.some(id=>RANK_ENCOUNTER_ROWS.find(e=>e.id===id).rank>o.rank)||![null,'B3','C1'].includes(o.familyBranch)||(o.familyBranch!==null&&o.cleared<42))return false;
...
/** The original compares a stage against `heroTotalPower`, the WHOLE roster's summed Power
 *  (readable/SceneUnderling.lua:606 labels the same field "all fighting"). `ladderPower` is that
 *  sum; it now lives in lib/adventure.mjs so the village campaign and the opening journey measure
 *  Power with one function against one table. Re-exported under its original name.
 *  Wrapped rather than aliased: lib/adventure.mjs and this module are in an import cycle, so binding
 *  `ladderPower` at module-evaluation time hits the temporal dead zone. Calling through defers it. */
export const openingPower=s=>ladderPower(s);
...
// 21 rows a chapter from chapter 1 (the opening's six chapters included), so the chapter follows from the
// count alone and does not depend on whether the late rows are loaded yet.
export const openingChapter=s=>Math.min(LAST_CHAPTER,Math.floor((s.opening?.cleared||0)/21)+1);
...
export function openingAction(s,action,target,value,income){
 ...
 try{
 switch(action){
 case 'openingBattle':{const st=OPENING_STAGES[o.cleared];if(!st&&openingPending(o.cleared))throw Error('The next chapters are still loading. Reconnect once if this persists; your progress is safe.');if(!st||st._id!==target)throw Error('Choose the next encounter.');stageStep(n,st,openingPower(s));break;}
 ...
 case 'openingAuto':{
  ...
  while(ran<cap){
   ...
   if(!st){stop=openingPending(run.opening.cleared)?'The next chapters are still loading. Reconnect once if this persists; your progress is safe.':'Every chapter is cleared.';break}
 ...
 case 'openingReforge':{const f=n.fellows[target];if(!f?.gear||f.aptitude>=APTITUDE_CAP)throw Error('Choose an equipped Fellow below the Aptitude cap.');changeItem(n,'Item_Quenching_Equipment_1',-1);f.aptitude++;n.fellows[target]=recordAptitude(f,'item:Item_Quenching_Equipment_1',1,1);o.reforges++;break;}
 ...
 else if(row.useEffect==='addAtk'){if(!n.fellows[value])throw Error('Choose a Fellow.');const f=n.fellows[value];const gain=row.useParam.count/100; if(f.aptitude+gain>APTITUDE_CAP)throw Error('Aptitude cap reached.');f.aptitude+=gain;n.fellows[value]=recordAptitude(f,'item:'+target,1,gain);}
 ...
 else if(target==='Item_Box_Talent_1'||target.startsWith('Item_Hero_Talent_Country_')){const f=n.fellows[value];if(!f||f.aptitude>=APTITUDE_CAP)throw Error('Choose a Fellow below the Aptitude cap.');f.aptitude++;n.fellows[value]=recordAptitude(f,'item:'+target,1,1);}
