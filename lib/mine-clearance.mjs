...
export function validMine(s){
 ...
 for(const r of m.history){
  if(!record(r)||r.id<=prevId||r.at<prevAt||r.policyVersion!==1||!(s.fellows?.[r.owner]||REMOVED.has(r.owner))||!int(r.power,1e15)||r.power===0||!int(r.before,TOTAL)||r.before===TOTAL||r.after!==Math.min(TOTAL,r.before+r.power)||!Array.isArray(r.kills))return false;
