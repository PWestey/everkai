...
// The table grew 8 -> 80 rows on 2026-09-15: the shipped eight came from a community page that
// stopped at eight, and are byte-identical to highwayBoss.json rows 1-8, so every stored receipt
// still revalidates unchanged. Orders 9-80 are new depth only.
export const MINE_ROWS=data.rows;
export const MINE_DEPTH=MINE_ROWS.length;
// highwayBoss has no name column and lang.json has no Boss_Mine_* key, so only the eight
// community-named rows have a name. Everything past them is labelled by its depth, not invented.
export const mineRowName=r=>r.name||`Mine guardian ${r.order}`;
// The original opens Mines & Golems at player level 12 (SystemUnlock row 8: PlayerLevel 12, plus a
// two-day server age that has no single-player meaning). Its first encounter, Rock Baby at 225,000
// Power, sits beside the original's chapter 20 boss (254,250), so an unlocked mine at rank 1-5 was a
// wall. Player rank is the original ladder in lib/opening.mjs.
export const MINE_UNLOCK_RANK=12;
export const mineUnlocked=s=>(s.opening?.rank||1)>=MINE_UNLOCK_RANK;
const DAY=86400000,LIMIT=10000,TOTAL=MINE_ROWS.at(-1).cumulativePower;
// The shipped mine was 8 rows deep until 2026-09-16, when highwayBoss.json's full 80 were imported.
// The eight original rows are byte-identical, so their per-kill rewards still revalidate -- but TOTAL
// is not a row, it is the SUM, and it moved from 3,530,000 to 41,678,127,000. Every receipt written by
// a Fellow strong enough to bottom out the old mine stored `after` clamped to the OLD total, and the
// clamp below recomputes it against the new one. That rejected real saves outright -- the import said
// 'no save migration' and was wrong, because it checked the rows and not the derived total.
// Accepting the exact historical cap is deliberately narrow: nothing else is admitted, so a tampered
// `after` still fails.
...
const clampOk=r=>r.after===Math.min(TOTAL,r.before+r.power)
 ||(r.after===LEGACY_TOTAL&&r.before+r.power>=LEGACY_TOTAL);
export function validMine(s){
 ...
 for(const r of m.history){
  if(!record(r)||r.id<=prevId||r.at<prevAt||r.policyVersion!==1||!(s.fellows?.[r.owner]||REMOVED.has(r.owner))||!int(r.power,1e15)||r.power===0||!int(r.before,TOTAL)||r.before===TOTAL||!clampOk(r)||!Array.isArray(r.kills))return false;
...
export function mineAction(s,action,id,value){
 ...
 if(!mineUnlocked(s))return fail(`Mine Clearance opens at player rank ${MINE_UNLOCK_RANK}.`);
 ...
 if(!int(p.power,1e15)||s.gold+p.gold>Number.MAX_SAFE_INTEGER||s.fellowXP+p.fellowEXP>MAX_FELLOW_XP||m.coins+p.coins>1e9)return fail('Reward storage is full. Deployment and damage were not spent.');
