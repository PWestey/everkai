...
import {HELPER_TASKS,helperState,helperBlocked,helperEnabled,helperNote,stellaOrder,bestPlant,bestGround,bestNegotiation,bestTreasureArea,bestInnDish,bestWorkshopJob,bestBanquetParty} from './helper.mjs';
import {stellaEntry,stellaPlan,stellaActivation} from './stella.mjs';
...
export const TODAY_ITEMS=[
 ...
 // WHERE THE SHARDS WENT. Stella is one shared pool per roster and the helper decides who climbs, so
 // this row names the ladder it will reach next and repeats the helper's own report of who it paid --
 // the run record, not a second ledger (lib/helper.mjs runStella, `did.notes`).
 {id:'stella',group:'play',label:'Spend your Stella shards',helper:['stella'],pays:'Fellow Power',
  probe(s){const order=stellaOrder(s);
   if(!order.length)return lock('No Fellow has a Stella track yet');
   const note=helperNote(s,'stella',todayKey(s));
   // stellaPlan(...,1) stops at the first affordable row, so this stays one row per Fellow, not forty.
   const ready=order.filter(id=>stellaEntry(s,id)?stellaPlan(s,id,1).rows.length>0:!!stellaActivation(id));
   if(!ready.length)return done(note?`Helper: ${note}`:'No shards the ladders can take');
   const next=fellowById(ready[0])?.name||ready[0];
   return todo(`${ready.length} ladder${ready.length===1?'':'s'} can take shards · next: ${next}${note?` · helper: ${note}`:''}`)}},
