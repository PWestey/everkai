...
import {REMOVED_COSTUMES} from './catalog.mjs';
...
export function pictureGate(r,s){
 ...
 // 33 of the 54 costume-gated pictures were gated on a costume the owner removed on 2026-09-17. The
 // picture itself was not removed -- its art still ships -- so the gate is dropped rather than the
 // picture becoming permanently unreachable, and a save that had already discovered one keeps loading
 // (validFamilyGallery re-checks the gate on every owned picture, so an unresolvable gate would refuse
 // the whole village). Only ids on the recorded removal list are forgiven; an id that never existed
 // still fails below, which is what keeps this from being a hole.
 if(id&&!REMOVED_COSTUMES.has(id)){
 ...
 }else if(!id&&r.gates.unlockType!==1)return 'Special unlock route not yet implemented';
