...
import {freshOpening} from './opening.mjs';
...
function roamOnce(s,r,roll){
 ...
 // Roaming's Fame also pays into the player-rank Fame the ladder spends (lib/opening.mjs).
 // A village without a journey yet gets one opened (freshOpening, what openingStart writes) so the Fame
 // this message announces is not dropped.
 const opening={...(s.opening||freshOpening()),fame:Math.min(1e9,(s.opening?.fame||0)+ROAM_FAME)};
 return {state:{...s,family,inventory,roaming:next,...(opening?{opening}:{})},message:`${roamLocation(e.location).name}: ${message} · Fame +${ROAM_FAME}`,entry};
}
