...
function runInnGuests(s,act,now){
 let steps=0;
 const guests=s.inn?.guests;
 if(!Array.isArray(guests))return {state:s,steps:0};
 for(const g of guests){
  if(steps>=CHORE_STEP_CAP)break;
  const id=typeof g==='string'?g:g?.id;
  if(!id)continue;
  const r=act(s,'serveInnSpecial',now,id,null);
...
function runExpo(s,act,now){
 let steps=0;
 const stalls=s.expo?.stalls;
 if(!stalls||typeof stalls!=='object')return {state:s,steps:0};
 for(const id of Object.keys(stalls)){
  if(steps>=CHORE_STEP_CAP)break;
  const r=act(s,'serveExpo',now,id,null);
  if(r&&!r.error&&r.state){s=r.state;steps++}
 }
 return {state:s,steps};
}
