...
export const HELPER_TASKS=[
 ...
 {id:'stella',    group:'progression', label:'Upgrade Stella with earned fragments',note:'Local chore: spends the shared shard pools on your highest-level Fellows first', run:runStella},
...
export function validHelper(s){
 ...
 if(h.did!==undefined){
  ...
  if(Object.keys(d).length!==2||typeof d.day!=='string'||d.day.length>10)return false;
...
export function helperAction(s,action,now,target,value,act){
 ...
 if(action==='helperRun'){
  ...
  let state=s;const done=[],skipped=[],didIds=[];
  for(const t of queue){
   if(t.run){const out=t.run(state,act,now);if(out.steps>0){state=out.state;done.push(`${t.label} (${out.steps})`);didIds.push(t.id)}else skipped.push(t.label);continue}
...
function runStella(state,act,now){
 ...
 // the same thing explicitly: highest level first, then current Power, then catalogue order.
 const tracked=Object.keys(d.s.fellows).filter(id=>crossoverStella(id)||STELLA_PROFILES.some(p=>p.id===id&&fellowById(id)?.type===p.type));
 const rank=id=>[d.s.fellows[id]?.level||0,bondedPower(d.s,id)];
 tracked.sort((a,b)=>{const [la,pa]=rank(a),[lb,pb]=rank(b);return lb-la||pb-pa});
