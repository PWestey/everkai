...
import {STAGES,teamPower,patrolCharges} from './adventure.mjs';
...
export const HELPER_TASKS=[
 ...
 {id:'campaign',  group:'progression', label:'Clear campaign stages and patrol',note:'Local chore: a stage pays back twice its fee and a patrol refunds its fee', run:runCampaign},
 {id:'stella',    group:'progression', label:'Upgrade Stella with earned fragments',note:'Local chore: a Fellow`s fragments buy only that Fellow`s Stella', run:runStella},
 ...
 // SPENDING, default OFF: it spends gold, so it runs only once the player has switched it on.
...
export function helperAction(s,action,now,target,value,act){
 ...
 if(action==='helperRun'){
  ...
  return {state:{...state,helper:{...hh,ranAt:now}},arrivals,
...
// STELLA ----------------------------------------------------------------------------------------------------
/** A Stella fragment is item-specific: Item_Owner_HeroPiece_<n> buys only hero_<n>'s Stella levels, and
 *  the ladder is a single fixed sequence. Activation is free. So activate every owned profile and buy every
 *  level the stock covers ('max'). */
function runStella(state,act,now){
 const d=driver(state,act,now);
 if(!d.s.fellows)return {state:d.s,steps:0};
 for(const p of STELLA_PROFILES){
  if(!d.room())break;
  if(!d.s.fellows[p.id]||fellowById(p.id)?.type!==p.type)continue;
  if(!stellaEntry(d.s,p.id)&&stellaActivation(p.id))d.fire('stellaActivate',p.id,{seq:stellaState(d.s).seq});
  if(stellaEntry(d.s,p.id)&&stellaPlan(d.s,p.id,'max').rows.length)d.fire('stellaUpgrade',p.id,{seq:stellaState(d.s).seq,count:'max'});
 }
 // The shared crossover shard track (lib/crossover-stella.mjs) is one PROFILE with many OWNERS, so it
 // cannot be reached by the loop above -- which reads each profile's id as its owner's id. Without this
 // the chore would silently skip every crossover Fellow while claiming to run Stella, and a dead chore
 // is worse than an absent one (docs/crossover-collection-plan.md 3.3.6). The pool is shared and the
 // ladder is per Fellow, so the fixed roster order spends it on the Fellows nearest the front rather
 // than spreading a partial level across all 133: one finished ladder is worth more than 133 unfinished
 // ones, because the flat bonus is the row the Fellow has actually reached.
 //
 // The ORIGINAL Fellows need no such loop even though 108 of them now share one item too: each of them
 // still has a profile of their own whose id is their own id (lib/hero-spirit.mjs), so the loop above
 // already reaches them -- and it spends that shared pool in the same front-of-roster order.
 for(const id of Object.keys(d.s.fellows)){
  if(!d.room())break;
  if(!crossoverStella(id))continue;
  if(!stellaEntry(d.s,id)&&stellaActivation(id))d.fire('stellaActivate',id,{seq:stellaState(d.s).seq});
...
// CAMPAIGN ----------------------------------------------------------------------------------------------------
/** The village campaign (lib/adventure.mjs). MEASURED: a stage charges cost 50n and pays gold 100n, so a
 *  clear returns twice its fee; a patrol charges 25n and refunds it in the same action. Neither can lower
 *  gold, so both are default-on. Battles only when the party's Power already meets the stage (a short
 *  party is refused before any gold moves). Patrols go to the highest cleared stage the party still beats,
 *  because patrol EXP is 40n. The party itself is the player's. */
function runCampaign(state,act,now){
 const d=driver(state,act,now);
 if(!d.s.adventure||!d.s.fellows)return {state:d.s,steps:0};
 while(d.room()){
  const next=STAGES.find(x=>x.id===d.s.adventure.cleared+1);
  if(!next||teamPower(d.s)<next.power||d.s.gold<next.cost||!d.fire('battle',next.id))break;
 }
 while(d.room()&&patrolCharges(d.s)>=1){
  const stage=[...STAGES].reverse().find(x=>x.id<=d.s.adventure.cleared&&teamPower(d.s)>=x.power&&d.s.gold>=25*x.id);
