...
import {FISH_SKILL_MAX,fishLevelCost,FISH,FISHING_GROUNDS,groundLevel,fishingLevel,fishingState,castKey,fishingIndex} from './fishing.mjs';
...
function runDuplicates(s,act,now){
 ...
 if(s?.fishing)while(room()){
  ...
  for(const id of g.displayed||[]){const level=g.skills?.[id]||1;if(level<FISH_SKILL_MAX&&level<pickLevel){pickLevel=level;pick=id}}
