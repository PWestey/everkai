...
export const ROSTER_SORTS=['default','power','aptitude','awakening','level','rarity','name'];
 ...
 *  @param {{power?:(id:string)=>number,level?:(id:string)=>number,rarityOf?:(id:string)=>string,keys?:Record<string,(id:string)=>number>}} [by] */
export function rosterSort(entries,owned={},mode='power',by={}){
 ...
 // `by.keys` lets a caller add its own sort without this module learning every system's name: the
 // Fellow roster's Aptitude and Awakening orders (docs/fellow-screen-specs/01-roster.md) arrive that
 // way, and `default` deliberately has no key so the catalogue order survives rosterOrder's grouping.
 const key=mode==='level'?(id=>Number(owned[id]?.level)||0)
  ...
  :by.keys?.[mode]||(mode==='default'?(_id=>0):(by.power||(_id=>0)));
