...
export function validFrontier(s){
 const f=s.frontier;if(f===undefined)return true;// Rule 12. This is a DERIVED save-validity condition on a table that just grew: it used to read
// `!==30` because 30 was the whole ladder. The ladder is now the original's 18,000 stages, so the
// first player to clear stage 31 would have had a perfectly good Frontier save refused by decode().
// `>=30` accepts strictly more saves than `!==30` did (every save it accepted still passes) and
// keeps the Frontier's meaning: it opens once the first five chapters are behind you.
if(!f||f.policyVersion!==1||!int(f.cleared,12)||!int(f.attempts,1e9)||!(s.adventure?.cleared>=30))return false;
...
export function frontierAction(s,action,target,value){
 ...
 if(s.gold+r.gold>MAX_GOLD||s.fellowXP+r.xp>MAX_FELLOW_XP||s.crystals+r.crystals>1e9)return fail('Spend currency or EXP to make room for the full victory reward.');
