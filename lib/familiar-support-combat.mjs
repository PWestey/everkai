// Combat v6: isolated evaluator; DOT then regeneration then expiry. Earlier versions stay frozen.
...
export function statusTargets(units,u,rule,floor,round){
 ...
 if(row?.startsWith('random'))pool.sort((a,b)=>hash(a)-hash(b)||a.slot-b.slot);else if(row==='lowestpercent')pool.sort((a,b)=>a.hp/a.HP-b.hp/b.HP||a.slot-b.slot);else if(row==='highestatk')pool.sort((a,b)=>effectiveAttack(b)-effectiveAttack(a)||a.slot-b.slot);else if(row==='highest')pool.sort((a,b)=>b.hp-a.hp||a.slot-b.slot);else if(!row||row==='lowest')pool.sort((a,b)=>a.hp-b.hp||a.slot-b.slot);
 return pool.slice(0,rule.targets);
}
export const supportAmount=(caster,target,rule)=>Math.floor((rule.basis==='targetMaxHP'?target.HP:rule.basis==='casterMaxHP'?caster.HP:caster.ATK)*rule.percent/100);
export function putStatus(target,caster,rule,round,name){
 if(target.hp<=0)return null;const key=caster.id+':'+rule.kind,entry={key,source:caster.id,kind:rule.kind,value:['vulnerable','attack','speed','dealt'].includes(rule.kind)?rule.percent:(['bleed','poison'].includes(rule.kind)?Math.floor(caster.ATK*rule.percent/100*(caster.outgoing??1)):supportAmount(caster,target,rule)),remaining:rule.turns,appliedRound:round,name};
...
export function tickStatuses(units,round,log){
 ...
 for(const u of units){if(u.hp<=0)continue;for(const s of u.statuses)if(round>s.appliedRound&&s.kind==='regen'){const damage=Math.min(u.HP-u.hp,s.value);u.hp+=damage;log.push({round,attacker:s.source,target:u.id,side:units.find(x=>x.id===s.source)?.side??u.side,skill:true,kind:'regen',name:s.name+' · regeneration',damage});}}
