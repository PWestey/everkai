...
// ECON-13: claimExpoStall was free, so all 35 stalls could be taken at once. The original pays for booths
// with a build currency earned by levelling the event: ESCRebornLevel.json grants buildCoin 1 at each of
// levels 1-12 of activity ESCReborn_01 (0 at 13-15), and ESCRebornEvent_BoothNormal.json gates each booth
// on unlockLevel, which is 1 for all four booths of that activity -- so the gate is the coin, not the level.
// Everkai's Expo runs 5 business days rather than 15 event levels, so its level analogue is a cleared day:
// one permit at the start (the original's level-1 coin) plus one per clear, i.e. at most 6 stalls instead
// of 35. Stage 1 needs 1 stall slot and stages 3-5 need 2, so the first permit always opens the event and
// the schedule can never deadlock. Both halves of "1 permit per level" come from ESCRebornLevel.json.
export const EXPO_STARTING_PERMITS=1;
export const expoPermits=s=>{const e=expoState(s);return EXPO_STARTING_PERMITS+e.clears.length-Object.keys(e.stalls).length};
// Deliberately NOT asserted in validExpo: real saves already hold stalls claimed for free under the old
// rule, and a validator would refuse to decode them. The gate lives on the action, where it bounds
// future claims without stranding anybody's village.
...
export function expoAction(s,action,target,value){
 ...
 if(action==='claimExpoStall'){
  if(!stalls.has(target)||Object.hasOwn(e.stalls,target))return fail('Choose a stall you do not own yet.');
  const permits=expoPermits(s);
  if(permits<1)return fail('No build permits left. Clear a business day to earn the next one.');
  return done({...e,stalls:{...e.stalls,[target]:1}},`Stall built for 1 build permit · ${permits-1} left.`);
 }
