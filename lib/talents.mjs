...
export const talentRule=id=>fellowById(id)?rules.get(sourceId(id))||null:null;
export const talentLevel=f=>f.talentLevel||0;
export const talentCap=(s,id)=>originalProgression(s)&&source.heroes[sourceId(id)]&&talentRule(id)?source.paidCap:talentRule(id)?.cap||0;
export function validTalentLedger(s,id,f){
 const h=f.originalTalent;if(h===undefined)return talentLevel(f)<=talentRule(id).cap;
 const r=talentRule(id);if(!originalProgression(s)||!source.heroes[sourceId(id)]||!h||h.policyVersion!==1||!Number.isInteger(h.baseline)||h.baseline<0||h.baseline>r.cap||!Array.isArray(h.receipts)||!h.receipts.length||h.receipts.length>299)return false;
