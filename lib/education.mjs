...
export function educationReward(s,p,rank,awards=1){const base=rank*10,percent=fishingEducationBonus(s,p.type),perAward=Math.floor(base*(100+percent)/100),total=perAward*awards;return {base,percent,perAward,total,credited:Math.min(total,Math.max(0,MAX_FELLOW_XP-s.fellowXP))}}
export const SCHOOL_TYPES=['inspiring','diligent','brave','informed','unfettered'];
export const GRADES={D:100,C:125,'B-':155,B:200,'B+':280};
/** The seats a village starts with; also the graduation threshold that opens the other two (ECON-16).
 *  Spelled here rather than imported from school.mjs (which holds the same 3 as MAX_PUPILS) because
 *  school.mjs already imports this module -- importing back closes the cycle and leaves SCHOOL_TYPES
 *  in its temporal dead zone at load. */
export const STARTING_SEATS=3,EXPANDED_SEATS=5;
export const schoolCapacity=s=>s.school.seats||STARTING_SEATS;
...
export function educationAction(s,action,target,value,rank){
 ...
 // ECON-16: this was a free, repeatable button that set seats to 5. No original price exists -- the
 // catalogue's own source column says so, and HeroEducateLevel.json is the original's hero-tempering
 // table (consume Item_Quenching_Hero_1/2), not a school-capacity table. So this is a GATE, not a price,
 // and the threshold is an existing constant rather than an invented one: MAX_PUPILS (3) is the starting
 // seat count, so the two extra seats open once a full starting cohort has actually graduated.
 if(action==='expandSchool'){
  if(schoolCapacity(s)>=EXPANDED_SEATS)return fail('Both extra school seats are already open.');
  if(s.school.graduates<STARTING_SEATS)return fail(`Graduate ${STARTING_SEATS} pupils to open two more seats. ${s.school.graduates} so far.`);
  return {state:{...s,school:{...s.school,seats:EXPANDED_SEATS}},message:'Two more school seats opened by your first graduating cohort.'};
 }
