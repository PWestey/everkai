...
export function artifactAction(s,action,target,value){
 ...
 if(!['claimOre','upgradeArtifact','upgradeArtifactMax','recycleArtifact','recycleTrackedArtifact'].includes(action))return null;
 const fail=error=>({state:s,error}),a=artifactState(s);
 if(action==='claimOre')return {state:{...s,artifacts:{...a,ore:Math.min(1e9,a.ore+1000)}},message:'Sandbox: 1,000 Magic Ore added (up to storage limit).'};
