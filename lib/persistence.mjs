import {SAVE_KEY,SAVE_VERSION,decode,fresh,settle,valid} from './game.mjs';
...
export function createPersistence(getStorage){
 ...
 function load(now){
  blocked=true;
  try{
   const storage=getStorage(),raw=storage.getItem(SAVE_KEY),base=raw?decode(raw):fresh(now);
