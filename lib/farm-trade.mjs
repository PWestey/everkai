...
export function essencePlan(s,essenceId,kind,amount=1,fellowId=null){
 ...
 const available=kind==='buy'?Math.min(Math.floor(t.dew/policy.essenceDewCost),1e9-owned):fellow&&fellowById(fellowId)?.type===essence.type?Math.min(owned,Math.floor((APTITUDE_CAP-fellow.aptitude)/policy.aptitudePerEssence)):0;
...
export function farmTradeAction(s,action,target,value){
 ...
 const fellow=s.fellows[target];if(!fellow||fellowById(target)?.type!==essence.type)return fail('Choose an owned Fellow of the matching type.');if(!(t.essences[essence.id]>0))return fail('Exchange Morning Dew for this essence first.');if(fellow.aptitude+policy.aptitudePerEssence>APTITUDE_CAP)return fail('This Fellow has reached the current Aptitude limit.');
 const plan=essencePlan(s,essence.id,'use',amount,target);return result({...t,essences:{...t.essences,[essence.id]:t.essences[essence.id]-plan.cost}},`Sandbox essence effect: Aptitude +${plan.gain}.`,{fellows:{...s.fellows,[target]:recordAptitude({...fellow,aptitude:fellow.aptitude+plan.gain},'essence:'+essence.id,plan.cost,plan.gain)}});
}
