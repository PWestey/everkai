...
/** How much of this plant has EVER been harvested -- the original's plants[id].recordCount, which is
 *  what gates the ladder. Saves written before the counter existed fall back to the stock on hand,
 *  a lower bound: it can only under-credit a player, never hand them a level they did not earn. */
export const farmPlantRecord=(f,id)=>f?.records?.[id]??f?.harvests?.[id]??0;
/** Cumulative harvest of this plant needed for its next level, or null at the terminal level 3.
 *  This is a REQUIREMENT, not a price: the original compares it against recordCount (the lifetime
 *  record) rather than count (the spendable stock), so reaching it spends nothing. */
export function farmUpgradeCost(f,id){const row=upgrades.get(id)?.levels[farmPlantLevel(f,id)-1];return row?.harvestsRequired??null;}
...
export function farmAction(s,action,target,value){
 ...
 if(action==='upgradePlant'){
  ...
  // the plant's LIFETIME harvest record, not the spendable stock and not a currency. So nothing is
  // deducted here: harvesting enough of a plant is itself the qualification.
  ...
  if(record<need)return fail(`Harvest ${(need-record).toLocaleString()} more ${name} to reach harvest level ${level+1}.`);
 ...
 // records is the original's plants[id].recordCount: a lifetime tally that orders never spend, which
 // is what the harvest-level ladder is measured against. harvests stays the spendable stock.
 const record=Math.min(1e9,farmPlantRecord(f,p.id)+p.amount);
