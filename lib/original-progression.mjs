...
export const sourceAptitudeBonus=(s,id)=>originalProgression(s)?data.heroes[sourceId(id)]-10+qualityRule(sourceQuality(s,id)).talent:0;
...
export function validOriginalProgression(s){
 ...
 if(Object.entries(s.fellows).some(([id,f])=>!data.heroes[sourceId(id)]||sourceQuality(s,id)!==(qualities[id]??1)||f.level>sourceCap(s,id)))return false;
...
export function originalProgressionAction(s,action,id){
 ...
 if(action==='activateOriginalProgression'){
  ...
  if(Object.keys(s.fellows).some(id=>!data.heroes[sourceId(id)]))return fail('An owned Fellow has no verified original growth record.');
