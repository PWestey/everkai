...
export const RESTORATION_MAX=20;
...
export function relicBonus(s){const bonus={aptitude:0,basicPowerPercent:0,powerPercent:0};
 for(const [id,r] of Object.entries(s?.treasure?.relics||{})){const e=relicEffect(id,r);if(e)bonus[e.stat]+=e.amount}
 return bonus;}
export function treasureAction(s,action,target,value){if(!['treasureRestore','treasureStart','treasureDig','treasureReturn','treasureAppraise','treasureDonate','treasureDisplay'].includes(action))return null;
 ...
 if(action==='treasureRestore'){const r=t.relics[target];if(!r?.donated)return fail('Donate this relic first.');const level=restorationLevel(r);if(level>=RESTORATION_MAX)return fail('This relic is fully restored.');const cost=level+1;if(r.materials<cost)return fail(`Requires ${cost} of this relic’s duplicate materials.`);
  ...
  const e=relicEffect(target,t.relics[target]);
  message=e?`Restored to ${level+1}/${RESTORATION_MAX} · ${relicById.get(target).name} now grants +${e.amount}${e.stat==='aptitude'?' Aptitude':'% Power'}.`:`Restored to ${level+1}/${RESTORATION_MAX} · this exhibit's original effect has no Everkai equivalent yet.`;}
