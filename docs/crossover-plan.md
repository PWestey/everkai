...
3. **Stop the counter selling them.** `recruitPrice → null` for additions; the Recruit panel says which
   ...
   with no other change; `summonRecruit` refuses by naming the arc (*"Hulk joins by playing Rooftop
   Watch, not at the counter."*) rather than *"no price is recorded"*; and the panel's tile and invite
   card both read `Story · <arc name>`. Save-safe as §4.4 of the storyline plan predicted: `validSummon`
   never re-derives a price, and a flag-on save holding a pre-change receipt still validates (pinned in
...
4. **Arcs and unlocks.** 33 arcs, 163 stages, blocks of five down the owner's rank order so his most
   ...
   from `selected-roster.json` so it cannot drift. **SHIPPED 2026-09-17** in
   `lib/crossover-arc-data.json`, generated by `scripts/crossover/build-arcs.mjs` from
   `lib/crossover-roster-data.json` (a verbatim mirror of the corpus's `selected-roster.json`, hashed).
   3,490 completions for the 163; the eight Isekai arcs stay at 10 and `lib/event-data.json` is
   untouched. Measured pace with the shipped journal: **first character day 1, all 163 day 60**
   (day 120 at half pace). `EVENTS` exports all 41 arcs so `validEvents` can check any save;
...
7. **Types (decision 3).** Assign by role, then spread the owner's top picks; report which characters
   ...
   `lib/everkai-additions-data.json` with `rarity:"N"` and their type's SSR anchor as `template`.
   Five rows moved, each with its reason in the data: Wolverine Brave→Unfettered, Hulk Brave→Diligent,
   Thor Brave→Inspiring, Ahsoka Brave→Diligent, Captain America (WWII) Inspiring→Diligent. Measured
   through `enterpriseBreakdown` at 26,000 workers, one operator slot is worth +1,560,006 gold/s in the
   best Brave building and +3,120,006 in the best Diligent one, so the top twenty by rank go from
