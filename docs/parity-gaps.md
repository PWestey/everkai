...
4. ~~**Offline accrual cap.**~~ **RESOLVED 2026-09-13.** Was `MAX_AWAY_MS = 8h`, a prototype value with no documented parity. The original's own ceiling is now recovered and matched: `System.json` carries `{"_id":"offlineMaxTime","integerValue":43200}` — **twelve hours** — and the private server clamps to the same figure (`village.py` `OFFLINE_LIMIT=43200`). `lib/game.mjs` now uses 12h, and the save dialog and DEVELOPMENT_HISTORY.md say so. Time beyond the window is discarded rather than banked, matching `village.py:63`, which advances the production cursor regardless of the clamp.

   One consequence worth recording rather than fixing blind: `settle` clamps `pending` at **1e9** (`lib/game.mjs:113`), so a longer window saturates that clamp at a *lower* income rate — 23,148 gold/s over 12h against 34,722 over 8h. Above that, away income is silently discarded. That clamp is part of the pending auto-gold decision, not this change.
