...
## Character art, clips and habit journal (September 2026)

Base and costume portraits were re-rendered at 1280×1920 and idle clips at 1024×1536, all in 2:3 framing, so the portrait character screen fills the frame without bars and landscape fills its side bars with a blurred still. Family clip scenes are now composited into every frame. The retired roster cutouts and Family scene backgrounds were removed from the offline bundle. The habit journal shows each task tab as one list under time-of-day headers, with multi-select on every task tab and review tasks listed under their own frequency. See SYSTEMS.md for measured settings and the open surround issue.
