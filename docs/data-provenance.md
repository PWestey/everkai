...
| `crossover-abilities-data.json` | 133 rows | local-invention | `scripts/crossover/build-abilities.mjs` | `localPolicy: crossover-archetypes-v1`; one archetype word per crossover Fellow, DERIVED from that row's own `occupation` by an ordered keyword rule. Naming only -- nothing numeric reads it | `lib/crossover-abilities.mjs`, `tests/crossover-abilities.test.mjs` | Regenerable; `--check` compares bytes. |
| `crossover-progression-data.json` | 8 badges | mixed | `scripts/crossover/build-abilities.mjs` | `localPolicy: crossover-progression-v1`. Every magnitude is the measured MINIMUM for its rarity badge across the original Fellows whose rarity chain STARTS there, re-derived from `original-progression-data.json` + `operation-data.json` at build time; the generator refuses to write unless it reproduces docs/crossover-abilities-plan.md 1.5/1.6. LOCAL: the selection rule (per-badge minimum), the LR row (repeats UR*) and the fixed talent rule | `lib/crossover-abilities.mjs`, `lib/talents.mjs`, `lib/insight.mjs`, `lib/operations.mjs`, `lib/original-progression.mjs`, `tests/crossover-abilities.test.mjs` | Crossover Fellows only; no original row moves. |
...
| `hero-bond-data.json` | 23 groups + 57 auras | sha-pinned | `import-hero-bond.py` | `HeroBond`/`HeroBondBonus`/`HeroBondCondition`/`SkillBase` sha-pinned; every skill id asserted to resolve | `lib/hero-bond.mjs`, `lib/hero-scope.mjs` | Collection half only. The group AURAS are imported and shown but NOT paid: their gate is `auraProgress` (UnderlingManager.lua:2157), a per-hero counter Everkai does not model. |
| `familiar-benefit-data.json` | 4 magnitudes + 1 daily | sha-pinned | `import-familiar-benefits.py` | `System.json`, `split_reward/rewardpetpacifydaliy.json` sha-pinned; importer asserts all four values | `lib/familiar-supplies.mjs`, `lib/familiar-explore.mjs` | Magnitudes and order are the original's (`System.PetBPRightShow`); the three GATES are Everkai's and are carried in the data's own `gates` block. |
| `familiar-dispatch-data.json` | 9 areas + fragment pools | sha-pinned | `import-familiar-dispatch.py` | `PetDispatch`/`split_reward/reward_petdispatch.json` sha-pinned | `lib/familiar-dispatch.mjs` | Great Success odds and the Power gate are the original's; the gate's meaning is flagged in 08-dispatch.md 7. |
| `familiar-exchange-data.json` | 1 exchange (of 10 rows) | sha-pinned | `import-familiar-exchange.py` | `ScoreExchange.json`, `split_reward/reward.json` sha-pinned | `lib/familiar-explore.mjs` | Only `PetShop_8` is ported; the nine Crystal-priced rows are recorded and deliberately unbuilt. The limit's RESET PERIOD is Everkai's (daily) -- no table states it. |
| `familiar-explore-data.json` | 3 areas, 70 pets, 5 lottery rows | sha-pinned | `import-familiar-explore.py` | `PetArea`/`PetCatchItem`/`PetExploreItem`/`PetExploreLottery`/`Pet`/`System` sha-pinned | `lib/familiar-explore.mjs` | Seeded RNG, MustCatch reading and the four buff magnitudes are local and marked in the module header. |
| `familiar-handbook-data.json` | 300 levels + 3 EXP tables | sha-pinned | `import-familiar-handbook.py` | `PetBookLevel`/`System`/`PetStar`/`Pet`/`PetGroup` sha-pinned; importer asserts the flat columns and the 300 country runs | `lib/familiar-handbook.mjs`, `lib/adventure.mjs` | The `PowerCoef` half only. The Crystal reward half is recorded and deliberately not ported. |
| `familiar-supply-data.json` | 2 items + hold hours | sha-pinned | `import-familiar-supplies.py` | `System.json` sha-pinned **VERIFIED** | `lib/familiar-supplies.mjs` | Star costs name no item in any table; Everkai charges class-up items and says so. |
| `familiar-tower-data.json` | 300 floors, 1,480 enemies, 23 bands | sha-pinned | `import-familiar-tower.py` | `PetTower`/`PetTowerArray`/`PetEndlessTower*`/`System`/`split_reward` sha-pinned | `lib/familiar-tower.mjs`, `lib/familiar-supplies.mjs` | Endless enemy draw and band stat scaling are Everkai's own repeatable rule (the original chose them server-side) and are marked. |
| `familiar-combat-data.json` | 192 skills + 137 buffs | sha-pinned | `import-familiar-combat.py` | `PetSkill`/`PetBuff`/`PetAttr`/`PetClass`/`Pet` sha-pinned; importer asserts the sparse column census and every skill/buff cross-reference | `lib/familiar-combat.mjs`, `lib/familiar-tower.mjs`, `lib/familiar-dispatch.mjs` | 329 rows that were 42% of all absent familiar data. Powers `PetInfo:GetPower`; the SkillType 4 stat modifiers reach POWER and deliberately NOT combat, which would need a combatVersion bump. |
| `familiar-portrait-data.json` | 286 sprites (143 head + 143 half) | sha-pinned | `import-familiar-portraits.py` | `UnityDataAssetPack.apk assets/Android/ui/pet`, FairyGUI v6 package `Pet_fui` + 27 atlas textures | `lib/familiar-portraits.mjs` | 66 of 71 familiars; the five without are absent from the original's `Pet.json` too and keep the rarity card. |
...
| *(no file)* `lib/crossover-stella.mjs` | 1 profile, 40 rows | mixed | *none — derived at load* | `localPolicy: crossover-shared-shard-v1`. The shared crossover shard ladder READS Angie's (`hero_52`) shipped `cost` and `flat` columns out of `stella-data.json` at import and zeroes `percent`, so it cannot drift from the table it came from. LOCAL: that one profile stands in for all 163, the zeroed percent, and the free `{0,0,0}` activation (the Elise precedent) | `lib/stella.mjs`, `lib/helper.mjs`, `tests/crossover-stella.test.mjs` | No new data file; the four original profiles are untouched. |
