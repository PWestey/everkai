...
app=Path(__file__).resolve().parents[1];p=WORKSPACE/'outputs/component-research/datasets/Item.json';rows={r['id']:r['en'] for r in json.loads(p.read_text())};id='Item_GetDE_10';key='Item:description:'+id;amount=int(re.search(r'obtain (\d+) Energy',rows[key]).group(1));assert amount==3
