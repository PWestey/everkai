...
test('both skill planners clip to Aptitude room without consuming for blocked effects',()=>{
 let s=go(fresh(1000),'recruit','hero_114');s.inventory.Item_Talent_Hero_1=100;s.fellows.hero_114.aptitude=998;assert.equal(talentTrainingPlan(s,'hero_114','max').count,0);assert.ok(act(s,'trainTalent',s.lastAt,'hero_114','max').error);s=stock(s,'hero_114');assert.equal(insightTrainingPlan(s,'hero_114','max').count,2);s=go(s,'trainInsight','hero_114','max');assert.equal(s.fellows.hero_114.aptitude,1000);assert.equal(s.insight.balances[insightRule('hero_114').materialId],800);const inv=structuredClone(s.inventory),insight=structuredClone(s.insight);assert.ok(act(s,'trainInsight',s.lastAt,'hero_114',5).error);assert.deepEqual(s.inventory,inv);assert.deepEqual(s.insight,insight);
});
