...
test('all110 exact talent identities spend only their supported cost and add only their effect',()=>{
 ...
 assert.equal(count,110,'158 before the 2026-09-17 roster trim took 48 Fellows out of the catalogue');assert.equal(s.inventory.Item_Talent_Hero_1,10000-spent);assert.equal(talentRule('hero_60'),null);assert.deepEqual(decode(JSON.stringify(s)),s);
});
test('UR and rarity-advance labels use the exact default node, while ambiguous and costume nodes refuse',()=>{
 // hero_114 and hero_142 carried this label and were deleted on 2026-09-17; hero_114 and hero_115 are
 // the same case -- UR Fellows whose default node is the +3 one -- and still ship.
 for(const id of ['hero_114','hero_115','hero_121'])assert.equal(talentRule(id).amount,3);assert.equal(talentRule('hero_186').amount,2);
 const p=guide.profiles.find(p=>p.id==='hero_114'),node=p.skills.find(n=>n.id==='Hero_Talent_Base_3');assert.ok(resolveTalentProfile(p));
...
test('both skill planners clip to Aptitude room without consuming for blocked effects',()=>{
 let s=go(fresh(1000),'recruit','hero_114');s.inventory.Item_Talent_Hero_1=100;s.fellows.hero_114.aptitude=APTITUDE_CAP-2;assert.equal(talentTrainingPlan(s,'hero_114','max').count,0);assert.ok(act(s,'trainTalent',s.lastAt,'hero_114','max').error);s=stock(s,'hero_114');assert.equal(insightTrainingPlan(s,'hero_114','max').count,2);s=go(s,'trainInsight','hero_114','max');assert.equal(s.fellows.hero_114.aptitude,APTITUDE_CAP);assert.equal(s.insight.balances[insightRule('hero_114').materialId],800);const inv=structuredClone(s.inventory),insight=structuredClone(s.insight);assert.ok(act(s,'trainInsight',s.lastAt,'hero_114',5).error);assert.deepEqual(s.inventory,inv);assert.deepEqual(s.insight,insight);
});
...
test('new talent training settles old income and preserves current Mine, Northern and Trading records',()=>{
 let s=go({...fresh(1000),opening:{...act(fresh(1000),'openingStart',1000).state.opening,rank:12}},'recruit','hero_114');s=stock(s,'hero_114');s.inventory.Item_Talent_Hero_1=60;s=go(s,'openEnterprise',BUSINESSES[0].id);s=go(s,'mineDeploy','hero_15',{seq:0,day:0});s=go(s,'northStart',null,{seq:0});s=go(s,'tradeBegin','learner',{seq:0,team:['hero_114']});const snapshots=structuredClone({mine:s.mineClearance,north:s.northern,trade:s.tradingPost}),expected=settle(s,2000),oldRate=totalRate(s);const r=act(s,'trainTalent',2000,'hero_114','max');assert.ok(!r.error,r.error);s=r.state;assert.equal(s.pending,expected.pending);assert.ok(totalRate(s)>oldRate);assert.deepEqual({mine:s.mineClearance,north:s.northern,trade:s.tradingPost},snapshots);assert.equal(s.fellows.hero_114.talentLevel,20);assert.equal(s.inventory.Item_Talent_Hero_1,0);
});
