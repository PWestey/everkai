...
import {talentRule,resolveTalentProfile,talentTrainingPlan} from '../lib/talents.mjs';import {insightRule,insightTrainingPlan} from '../lib/insight.mjs';import {createPersistence} from '../lib/persistence.mjs';
...
test('both skill planners clip to Aptitude room without consuming for blocked effects',()=>{
 let s=go(fresh(1000),'recruit','hero_113');s.inventory.Item_Talent_Hero_1=100;s.fellows.hero_113.aptitude=998;assert.equal(talentTrainingPlan(s,'hero_113','max').count,0);assert.ok(act(s,'trainTalent',s.lastAt,'hero_113','max').error);s=go(s,'claimInsight','hero_113');assert.equal(insightTrainingPlan(s,'hero_113','max').count,2);s=go(s,'trainInsight','hero_113','max');assert.equal(s.fellows.hero_113.aptitude,1000);assert.equal(s.insight.balances[insightRule('hero_113').materialId],800);const inv=structuredClone(s.inventory),insight=structuredClone(s.insight);assert.ok(act(s,'trainInsight',s.lastAt,'hero_113',5).error);assert.deepEqual(s.inventory,inv);assert.deepEqual(s.insight,insight);
});
