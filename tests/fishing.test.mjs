...
test('failed cast or research write cannot admit partial bait spend or duplicate reward',()=>{
 ...
 assert.equal(p.current.fishing.catches.length,7);assert.equal(p.current.fishing.bait,13);assert.equal(p.current.fishing.points,1);
});
...
test('new effect display, research upgrade and removal settle income at previous rate',()=>{
 let s=run(fresh(1000),'openEnterprise','Building_101');for(let n=0;n<88;n++){if(!fishingState(s).bait)s=run(s,'claimBait');s=run(s,'castFish',castKey(s));}
