import test from 'node:test';import {withCatches} from './fishing-helpers.mjs';import assert from 'node:assert/strict';
...
const run=(s,a,id=null)=>{const r=act(s,a,s.lastAt,id,a==='castFish'?'Village River':null);assert.ok(!r.error,r.error);assert.ok(valid(r.state));return r.state};
...
test('new effect display, research upgrade and removal settle income at previous rate',()=>{
 let s=run(fresh(1000),'openEnterprise','Building_101');s=withCatches(s,88);
...
// CHANGED 2026-09-18 (docs/power-sources-import-spec.md 2.5): a catch's stored effect is a RECEIPT. Power reads
// the species' own original skill (Fish_1103: +3,000 to every Fellow, +1,000 a level) at power time, so a
// historical or tampered stored number (1,234 / +7 here) no longer reaches Power -- it is still stored,
// validated and round-tripped exactly as before.
test('stored effect values are receipts; Power reads the species’ own original skill',()=>{
 let s=catchMany(6);s.fishing.catches=s.fishing.catches.map(c=>c.fish==='F1103'?{...c,flat:1234,increment:7}:c);s=run(s,'displayFish','F1103');assert.equal(fishingBonus(s,'hero_15'),3000);for(const id of ['catch:4','catch:5'])s=run(s,'researchFish',id);s=run(s,'upgradeFish','F1103');assert.equal(fishingBonus(s,'hero_15'),4000);s=decode(JSON.stringify(s));assert.equal(s.fishing.catches.find(c=>c.fish==='F1103').flat,1234,'the receipt is untouched');s=run(s,'removeFish','F1103');s=run(s,'displayFish','F1103');assert.equal(fishingBonus(s,'hero_15'),4000);
 const current=structuredClone(s);current.fishing.catches=current.fishing.catches.map(c=>({id:c.id,fish:c.fish,name:c.name,caughtAt:c.caughtAt,duplicate:c.duplicate,policyVersion:2,effect:{kind:'flat',type:c.type,rarities:[],initial:c.flat,increment:c.increment}}));assert.ok(valid(current));assert.equal(fishingBonus(current,'hero_15'),4000);assert.deepEqual(decode(JSON.stringify(current)),current);
});
