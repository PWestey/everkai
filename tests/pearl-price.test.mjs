...
test('the Skill Pearl price starts at 200 gold and doubles every 2,500 pearls bought',()=>{
 ...
 assert.equal(pearlPrice(65_000),Math.round(200*2**26),'a 66-Fellow roster at Aptitude 1000 ends near 13.4B a pearl');
...
test('buying charges the ladder and records lifetime purchases; an old save starts at the base price',()=>{
 ...
 assert.equal(s.shopPearls,25);assert.equal(1e12-s.gold,Array.from({length:25},(_,n)=>pearlPrice(n)).reduce((a,b)=>a+b),'each pearl is charged its own rung');assert.equal(1e12-s.gold,5015);
 s={...s,shopPearls:PEARL_PRICE_DOUBLING*4};
 const before=s.gold;s=run(s,'buySupply',PEARL_ID,1);
 assert.equal(before-s.gold,3200,'the 10,001st pearl costs 16x the base');
