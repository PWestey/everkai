...
test('Frontier advice respects saved party, used leaders and future-only cap projection',()=>{
 ...
 for(const id of ['hero_1','hero_105'])s.fellows[id].level=100;s.inventory.local_limit_token=1;s.inventory.Item_Talent_Hero_1=0;const cap=encounterAdvice(s,true).options.find(o=>o.action==='limitBreak');assert.ok(cap);const r=act(s,cap.action,1000,cap.target,cap.value);assert.equal(r.state.fellows[cap.target].level,100);assert.equal(r.state.fellowXP,s.fellowXP);assert.match(cap.detail,/button only raises the cap/);
});
