import test from 'node:test';import assert from 'node:assert/strict';
import {createHash} from 'node:crypto';
import {fresh,act,valid,decode} from '../lib/game.mjs';
import {OPENING_STAGES,CAMPAIGN,LAST_CHAPTER,OPENING,decodeCampaign,openingBossReady} from '../lib/opening.mjs';
import {openingChapterName} from '../lib/opening-presentation.mjs';
import main from '../lib/campaign-chapters-data.json' with {type:'json'};
import lateRaw from '../lib/campaign-chapters-late-data.json' with {type:'json'};
// Chapters 7-3000 ship in the main file, 3001-6000 in the lazily loaded late file (lib/stage-ladder.mjs).
// `raw` is the two joined back into the one format-2 table, which is what every pin below is over.
const {late:_index,...mainRows}=main;
const raw={...mainRows,lastChapter:lateRaw.lastChapter,battles:[...main.battles,...lateRaw.battles],bosses:[...main.bosses,...lateRaw.bosses],backgrounds:[...main.backgrounds,...lateRaw.backgrounds]};
import scenes from '../lib/stage-scene-data.json' with {type:'json'};

const chapterOf=s=>Number(s._id.split('-')[0]);

test('the stage ladder runs chapters 1-6000: 126,000 stages, 21 a chapter, stageIds without gaps',()=>{
 assert.equal(LAST_CHAPTER,6000);
 assert.equal(OPENING_STAGES.length,6000*21,'126 opening stages + 125,874 imported');
 OPENING_STAGES.forEach((s,i)=>{if(s.stageId!==i+1)assert.fail(`stage ${i+1} has stageId ${s.stageId}`)});
 const byChapter=new Map();for(const s of OPENING_STAGES){const c=chapterOf(s);if(!byChapter.has(c))byChapter.set(c,[]);byChapter.get(c).push(s);}
 for(let c=1;c<=6000;c++){const rows=byChapter.get(c)||[];
  assert.equal(rows.length,21,'chapter '+c);assert.equal(rows.filter(s=>s.boss).length,1);assert.equal(rows.at(-1)._id,c+'-6-0');}
 assert.equal(CAMPAIGN.battles.length,5994*20);assert.equal(CAMPAIGN.bosses.length,5994);});

test('imported rows are the original values, continuing the opening curve',()=>{
 const at=id=>OPENING_STAGES.find(s=>s._id===id);
 // Measured from BattleNormal/LevelBoss: chapter 6 boss 23,100; chapter 7's first stage is 10,000 and its boss is
 // 27,850; chapter 20's boss is 254,250, the neighbour of Mine Clearance's first guardian (225,000).
 assert.equal(at('6-6-0').atk,23_100);assert.equal(at('7-1-1').atk,10_000);assert.equal(at('7-6-0').atk,27_850);assert.equal(at('20-6-0').atk,254_250);
 // Chapters 51-150, measured from the same tables: 50-6-0 1,513,000; 51-1-1 557,500; 100-6-0 3,725,000;
 // 150-1-1 2,411,000; 150-6-0 6,698,000 (stageId 3150).
 assert.equal(at('50-6-0').atk,1_513_000);assert.equal(at('51-1-1').atk,557_500);assert.equal(at('100-6-0').atk,3_725_000);
 assert.equal(at('150-1-1').atk,2_411_000);assert.deepEqual([at('150-6-0').atk,at('150-6-0').stageId],[6_698_000,3150]);
 // Chapters 151-3000: 1500-6-0 394,900,000; 3000-1-1 428,000,000; 3000-6-0 1,191,000,000 (stageId 63000).
 assert.equal(at('1500-6-0').atk,394_900_000);assert.equal(at('3000-1-1').atk,428_000_000);assert.deepEqual([at('3000-6-0').atk,at('3000-6-0').stageId],[1_191_000_000,63000]);
 // Chapters 3001-6000, measured from the same tables: 3001-1-1 429,000,000; 4322-6-0 3,501,000,000 (the first boss
 // above the owner's real original roster, 3,497,276,469); 6000-1-1 18,940,000,000; 6000-6-0 52,620,000,000.
 assert.equal(at('3001-1-1').atk,429_000_000);assert.equal(at('4322-6-0').atk,3_501_000_000);assert.equal(at('6000-1-1').atk,18_940_000_000);
 assert.deepEqual([at('6000-6-0').atk,at('6000-6-0').stageId],[52_620_000_000,126000]);
 assert.ok(OPENING_STAGES.every(s=>Number.isSafeInteger(s.atk)),'every atk is an exact integer (rule 5), so BigInt(atk) in battleQuote is exact');
 const items=new Set(OPENING.items.map(i=>i._id));
 for(const b of CAMPAIGN.bosses)for(const i of b.items)assert.ok(items.has(i.id),`${b._id} rewards ${i.id}, which has no item definition`);
 assert.deepEqual(CAMPAIGN.provenance.droppedBossItems,['Item_Weapon_Equipment_1_2','Item_Weapon_Equipment_2_1','Item_Weapon_Equipment_2_2','Item_Weapon_Equipment_3_1']);
 assert.ok(CAMPAIGN.battles.every(b=>!b.stageEventId),'imported stages carry no roadside events (deferred; see the import script)');
 assert.equal(CAMPAIGN.battles.filter(b=>b.sourceEventId).length,11988,'source event ids are kept for later: 88 in 7-50, 200 in 51-150, 5,700 in 151-3000, 6,000 in 3001-6000');
 assert.equal(CAMPAIGN.battles.filter(b=>b.mushRoomType===null).length,1,'the one source row without mushRoomType is kept as null');});

// sha256 of JSON.stringify({battles,bosses,backgrounds}) for chapters 7-50 as format 1 shipped them (167b3e9);
// the import script pins the same digest.
const SHIPPED_7_50='4d9d798606140f30efffe11e21c8b33ce9ef5339f7b8bae7fa1dac87a553f111';
const digest7to50=c=>{const old=r=>chapterOf(r)<=50;return createHash('sha256').update(JSON.stringify({battles:c.battles.filter(old),bosses:c.bosses.filter(old),
 backgrounds:Object.fromEntries(Object.entries(c.backgrounds).filter(([k])=>Number(k)<=50))})).digest('hex')};

test('chapters 7-50 decode to exactly the rows that shipped before the compact format',()=>{
 assert.equal(CAMPAIGN.provenance.chapters7to50Sha256,SHIPPED_7_50);
 assert.equal(digest7to50(CAMPAIGN),SHIPPED_7_50);
 // Negative controls: one changed value, or a reordered key, must change the digest.
 const bumped=structuredClone(raw);bumped.battles[13*20+5][2]++;
 assert.notEqual(digest7to50(decodeCampaign(bumped)),SHIPPED_7_50,'a changed atk in chapter 20 is detected');
 const reordered=decodeCampaign(raw);reordered.bosses[0]={items:reordered.bosses[0].items,...reordered.bosses[0]};
 assert.notEqual(digest7to50(reordered),SHIPPED_7_50,'key order is part of the pinned shape');
 assert.throws(()=>decodeCampaign({...raw,format:1}),/Unknown campaign data format/);});

// The 6,000-chapter extension must leave every row a596db8 shipped untouched. Same digest scheme as above,
// over chapters 7-3000 as that commit decoded them (its data file's sha256 was 3d9c029f...b702); the import
// script pins the same value. ALL_ROWS is the same digest over every imported row, 7-6000, as
// scripts/import-campaign-chapters.py computed it from BattleNormal/LevelBoss/Chapter.json -- the coverage
// guard over the whole table: any changed, missing or extra row moves it.
const SHIPPED_7_3000='6ba956d05510875f4fe1b7c111543567e8a83faa3d2708c56b14582a34647d4e';
const ALL_ROWS='42e984b718fce58f4ccfbfaa1bdc0499866c16c99db4dada9109f9d796ae9444';
const digestTo=(c,last)=>{const old=r=>chapterOf(r)<=last;return createHash('sha256').update(JSON.stringify({battles:c.battles.filter(old),bosses:c.bosses.filter(old),
 backgrounds:Object.fromEntries(Object.entries(c.backgrounds).filter(([k])=>Number(k)<=last))})).digest('hex')};

test('chapters 7-3000 are exactly what shipped before the extension, and every imported row is pinned',()=>{
 assert.equal(CAMPAIGN.provenance.chapters7to3000Sha256,SHIPPED_7_3000);
 assert.equal(digestTo(CAMPAIGN,3000),SHIPPED_7_3000);
 assert.equal(CAMPAIGN.provenance.allRowsSha256,ALL_ROWS);
 assert.equal(digestTo(CAMPAIGN,CAMPAIGN.lastChapter),ALL_ROWS);
 // Negative controls. A change past chapter 3000 moves the all-rows pin but not the shipped one; a change
 // inside it moves both; dropping the last chapter moves the all-rows pin.
 const late=structuredClone(raw);late.battles[(5000-7)*20+3][2]++;const lateC=decodeCampaign(late);
 assert.equal(digestTo(lateC,3000),SHIPPED_7_3000);assert.notEqual(digestTo(lateC,lateC.lastChapter),ALL_ROWS,'a changed atk in chapter 5000 is detected');
 const early=structuredClone(raw);early.bosses[2000-7][1]++;const earlyC=decodeCampaign(early);
 assert.notEqual(digestTo(earlyC,3000),SHIPPED_7_3000,'a changed boss inspire in chapter 2000 is detected');
 const short={...raw,lastChapter:5999};const shortC=decodeCampaign(short);
 assert.notEqual(digestTo(shortC,shortC.lastChapter),ALL_ROWS,'a missing chapter is detected');});

test('decoded rows keep the opening row shape and derive ids from position',()=>{
 const keys=Object.keys(OPENING.battles[0]).join();
 for(const b of CAMPAIGN.battles)assert.equal(Object.keys(b).filter(k=>k!=='sourceEventId').join(),keys,b._id);
 assert.deepEqual(Object.keys(CAMPAIGN.bosses[0]),['_id','atk','inspireConsumeBase','stageId','items']);
 assert.deepEqual(OPENING_STAGES.find(s=>s._id==='73-4-3').consume.map(c=>c.id),['3']);
 assert.equal(raw.format,2);assert.ok(JSON.stringify(raw).length<5_100_000,'6,000 compact chapters stay under 5.1 MB (measured 5,059,748 bytes; 1,141,681 gzip -9): see scripts/import-campaign-chapters.py for why 12,000 is not');
 // Only chapters 7-3000 are in the main file, which is inlined into the precached main chunk; 3001-6000 load lazily.
 assert.equal(main.lastChapter,3000);assert.ok(JSON.stringify(main).length<2_450_000,'the main-bundle half stays at the 3,000-chapter size (2,406,847 bytes)');});

test('every chapter background resolves to a shipped stage scene',()=>{
 const shipped=new Set(Object.values(scenes.chapters).map(c=>c.background));
 assert.equal(Object.keys(CAMPAIGN.backgrounds).length,5994);
 for(const [n,b] of Object.entries(CAMPAIGN.backgrounds))assert.ok(shipped.has(b.shown),`chapter ${n}: ${b.source} shows ${b.shown}, which has no scene art`);
 assert.deepEqual(CAMPAIGN.backgrounds['59'],{source:'Bg_Mountain_01',shown:'Bg_Level_03'});
 // Negative control: an unmapped source decodes to no scene.
 const broken=structuredClone(raw);delete broken.backgroundArt.Bg_Mountain_01;
 assert.equal(shipped.has(decodeCampaign(broken).backgrounds['59'].shown),false);});

test('clearing chapter 6 opens chapter 7, and a save past stage 126 is valid',()=>{
 let s=act(fresh(0),'openingStart',0).state;
 s.opening.cleared=125;s.fellows.hero_15.aptitude=1000;s.gold=1e9;
 let r=act(s,'openingBattle',0,'6-6-0');assert.equal(r.error,undefined,r.error);s=r.state;
 assert.equal(OPENING_STAGES[s.opening.cleared]._id,'7-1-1');
 r=act(s,'openingBattle',0,'7-1-1');assert.equal(r.error,undefined,r.error);s=r.state;
 assert.equal(s.opening.cleared,127);assert.ok(valid(s));assert.deepEqual(decode(JSON.stringify(s)),s);
 assert.equal(openingChapterName(7),'Chapter 7');
 s.opening.cleared=OPENING_STAGES.length+1;assert.equal(valid(s),false,'cannot clear past the last stage');});

test('clearing chapter 50 opens chapter 51, and the ladder runs through chapter 3000 into 3001 and on to 6000',()=>{
 let s=act(fresh(0),'openingStart',0).state;
 // Aptitude 1,000 at level 50 is 10,800,000 Power, above the 6,698,000 of chapter 150's boss.
 s.opening.cleared=1049;s.fellows.hero_15.aptitude=1000;s.fellows.hero_15.level=50;s.gold=1e14;
 let r=act(s,'openingBattle',0,'50-6-0');assert.equal(r.error,undefined,r.error);s=r.state;
 assert.equal(OPENING_STAGES[s.opening.cleared]._id,'51-1-1');
 // One fully trained Fellow is 301,600,000 Power; four clear chapter 3000's 1,191,000,000 boss, and the
 // ladder no longer ends there: chapter 3001 is next.
 s.opening.cleared=62999;s.gold=1e14;for(const id of ['hero_15','hero_1','hero_4','hero_3'])s.fellows[id]={level:750,aptitude:1000,skill:20,breaks:13,gear:null};
 assert.ok(valid(s));
 r=act(s,'openingBattle',0,'3000-6-0');assert.equal(r.error,undefined,r.error);s=r.state;
 assert.equal(s.opening.cleared,63000);assert.equal(OPENING_STAGES[s.opening.cleared]._id,'3001-1-1');
 r=act(s,'openingBattle',0,'3001-1-1');assert.equal(r.error,undefined,r.error);s=r.state;
 assert.equal(s.opening.cleared,63001);assert.ok(valid(s));assert.deepEqual(decode(JSON.stringify(s)),s);
 // Chapter 6000's boss (52,620,000,000) is above every roster this build can field: all 111 Fellows fully
 // trained sum to 33,477,600,000, whose first wall is chapter 5631's 33,480,000,000 boss. That is the point
 // of the extension -- the ladder now runs ahead of Power -- so the top is exercised by the rule and the
 // save bound rather than by a legal roster.
 const top=OPENING_STAGES.at(-1);assert.equal(top._id,'6000-6-0');
 assert.equal(openingBossReady(top,top.atk),false);assert.equal(openingBossReady(top,top.atk+1),true);
 s.opening.cleared=OPENING_STAGES.length;assert.equal(OPENING_STAGES[s.opening.cleared],undefined);
 assert.ok(valid(s));assert.deepEqual(decode(JSON.stringify(s)),s);assert.equal(openingChapterName(6000),'Chapter 6000');
 assert.ok(act(s,'openingBattle',0,'6000-6-0').error,'nothing to fight past the last stage');});
