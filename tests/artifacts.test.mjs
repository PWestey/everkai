...
test('artifact upgrades use ore and affect equipped Fellow only, persist through transfers',()=>{
 ...
 assert.ok(run(s,'upgradeArtifact','hero_15').error);s=run(s,'claimOre').state;s=run(s,'upgradeArtifact','hero_15').state;
...
test('level cap and inventory limits cannot consume ore or lose equipped copies',()=>{
 let s=run(fresh(1000),'sandboxAdventure').state;s=run(s,'equip','hero_15',club).state;s=run(s,'claimOre').state;
