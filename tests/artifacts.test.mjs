...
import {supplied} from './gear-fixtures.mjs';
...
test('artifact upgrades use ore and affect equipped Fellow only, persist through transfers',()=>{
 let s=supplied(fresh(1000));s=run(s,'equip','hero_15',club).state;
...
test('old saves retain base equipment; invalid instance metadata is rejected',()=>{
 ...
 s=supplied(s);s=run(s,'equip','hero_15',club).state;
...
test('level cap and inventory limits cannot consume ore or lose equipped copies',()=>{
 let s=supplied(fresh(1000));s=run(s,'equip','hero_15',club).state;s={...s,artifacts:{...(s.artifacts||{ore:0,bag:{}}),ore:1000}};
