...
// A retired private fragment, stocked the way a save written before 2026-09-19 holds it: a grant row.
const grantLegacy=(s,item,count=1000)=>{const t=stellaState(s),seq=t.seq+1;return {...s,stella:{...t,seq,stock:{...t.stock,[item]:(t.stock[item]||0)+count},grants:[...t.grants,{id:seq,itemId:item,count,at:s.lastAt}]}};};
...
test('eligibility, activation, cross-fragment spending, stale and batch affordability boundaries',()=>{let s=fresh(1000);assert.ok(result(s,'stellaActivate').error);s=setup();
 ...
 assert.ok(result(s,'stellaActivate','hero_105').error);assert.ok(result(s,'stellaUpgrade').error);s=go(s,'stellaActivate');assert.ok(result(s,'stellaActivate').error);assert.ok(result(s,'stellaUpgrade').error);
 // Cross-fragment spending, 2026-09-19: every ladder spends the village pool now, so the item that must NOT
 // cross is a retired PRIVATE fragment -- hero_56's old stock buys only hero_56's ladder.
 s=grantLegacy(s,'Item_Owner_HeroPiece_56');assert.ok(result(s,'stellaUpgrade').error,'hero_54 cannot spend hero_56’s private fragments');s=grantFragments(s);const plan=stellaPlan(s,'hero_54',5);s=go(s,'stellaUpgrade','hero_54',5);assert.equal(stellaEntry(s,'hero_54').level,5);assert.equal(s.stella.stock.Item_Owner_VillageShard,1000-plan.cost);assert.equal(s.stella.stock.Item_Owner_HeroPiece_56,1000,'and it was not touched');const seq=s.stella.seq;s=go(s,'stellaUpgrade','hero_54','max');assert.ok(act(s,'stellaUpgrade',s.lastAt,'hero_54',{seq}).error);assert.ok(stellaPlan(s,'hero_54',1).rows.length===0);});
test('failed activation/upgrade persists fragments and exact retry once; receipts survive offline',()=>{let s=setup();s=grantFragments(s);let raw=JSON.stringify(s),fail=false;const p=createPersistence(()=>({getItem:()=>raw,setItem:(_,v)=>{if(fail)throw Error('quota');raw=v}}));p.load(1000);fail=true;assert.throws(()=>p.commit(go(p.current,'stellaActivate')));assert.equal(stellaEntry(p.current,'hero_54'),null);fail=false;p.load(1000);p.commit(go(p.current,'stellaActivate'));const prior=structuredClone(p.current.stella);fail=true;assert.throws(()=>p.commit(go(p.current,'stellaUpgrade','hero_54',5)));assert.deepEqual(p.current.stella,prior);fail=false;p.load(1000);p.commit(go(p.current,'stellaUpgrade','hero_54',5));const final=structuredClone(p.current.stella);p.load(86400000);
 ...
 const gained=(p.current.stella.stock.Item_Owner_VillageShard||0)-(final.stock.Item_Owner_VillageShard||0);
 assert.equal(gained,p.current.stella.idle.Item_Owner_VillageShard,'every fragment gained is recorded as idle');
...
test('malformed fragment conservation, owner, sequential level and scope rejected; old receipt values not repriced',()=>{let s=grantFragments(go(setup(),'stellaActivate'));s=go(s,'stellaUpgrade');for(const f of [t=>t.stock.Item_Owner_VillageShard++,t=>t.history[1].owner='hero_102',t=>t.history[1].level=5,t=>t.history[1].type='Diligent']){const bad=structuredClone(s);f(bad.stella);assert.equal(valid(bad),false);assert.throws(()=>decode(JSON.stringify(bad)));}s.stella.history[1].paid=21;s.stella.stock.Item_Owner_VillageShard--;s.stella.history[1].flat=499999;assert.deepEqual(decode(JSON.stringify(s)).stella,s.stella);});
