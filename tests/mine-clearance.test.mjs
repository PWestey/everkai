...
// REBASELINED 2026-09-18. Every figure below that depends on `ready()`'s Fellow moved, and none of the
// mine's own rules did. `ready()` maxes hero_54's Stella, and lib/stella.mjs applyStella now adds the
// owner's flat Power AFTER the typed multiplier instead of inside it -- the original's own order
// (private-server/readable/PropManager.lua:116), which Everkai had backwards and its own UI admitted was
// unverified. hero_54 there goes 23,124,147 -> 10,300,147 Power, so she clears 13 rows a day instead of
// 15 and earns proportionally fewer coins. Old SAVES are unaffected: validMine checks a receipt against
// the power STORED in it, never a recomputation, which tests/crossover-stella.test.mjs pins with a save
// written by an older build.
test('fresh earned route across four days clears exact encounters, exchanges20Ore and upgrades without freeOre',()=>{
 let s=ready();assert.equal(minePlan(s,'hero_54').kills.length,13);const gold=s.gold,xp=s.fellowXP,energies=structuredClone({energy:s.energy,adventure:s.adventure,tradingPost:s.tradingPost});
 for(let d=0;d<4;d++){s=settle(s,d*DAY+1000);s=mine(s,'mineDeploy');s=mine(s,'mineExchange','hero_54','max');}
 assert.equal(s.gold-gold,780000);assert.equal(s.fellowXP-xp,146400);assert.equal(s.mineClearance.history.length,4);assert.equal(s.mineClearance.coins,2320);assert.equal(s.artifacts.ore,20);
...
test('shop exact price, shared daily5cap, Ore capacity and historical price retention',()=>{
 let s=ready();for(let d=0;d<2;d++){s=settle(s,d*DAY+1000);s=mine(s,'mineDeploy');}s=mine(s,'mineExchange','hero_54','max');assert.equal(s.artifacts.ore,5);assert.equal(s.mineClearance.coins,2660);assert.equal(mineToday(s).bought,5);assert.ok(act(s,'mineExchange',s.lastAt,null,{seq:3,day:1}).error);
...
test('interrupted final reward retains Power damage, participation and wallets until explicit retry',()=>{
 let s=ready();s=mine(s,'mineDeploy','hero_15');let raw=JSON.stringify(s),fail=false;const p=createPersistence(()=>({getItem:()=>raw,setItem:(_,v)=>{if(fail)throw Error('quota');raw=v}}));p.load(1000);const before=structuredClone(p.current);fail=true;assert.throws(()=>p.commit(mine(p.current,'mineDeploy')));assert.deepEqual(p.current,before);assert.deepEqual(JSON.parse(raw),before);fail=false;p.load(1000);p.commit(mine(p.current,'mineDeploy'));p.load(1000);assert.equal(mineToday(p.current).cleared,13);assert.equal(p.current.mineClearance.coins,2080);assert.equal(p.current.mineClearance.history.length,2);
});
