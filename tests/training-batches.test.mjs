...
test('max talents equal singles and preserve prior Aptitude at source and resource boundaries',()=>{
 ...
 s.fellows.hero_15.aptitude=APTITUDE_CAP-1;assert.equal(talentTrainingPlan(s,'hero_15').count,Math.floor(1/r.amount));s.fellows.hero_15.aptitude=100;s.inventory.Item_Talent_Hero_1=r.cost*2-1;assert.equal(talentTrainingPlan(s,'hero_15').count,1);
});
