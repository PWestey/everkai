...
test('Knowledge expands plots; a ripened crop awards no extra global time or repeats',()=>{
 let s=run(fresh(1000),'openFarm');for(let n=0;n<4;n++){s=run(s,'sowFarm',0,'Plant1');s=run(s,'waterFarm',0);s=ripe(s);assert.throws(()=>act(s,'finishFarm',1000,0),/Unknown action/,'instant maturation is retired');s=run(s,'harvestFarm',0);}
