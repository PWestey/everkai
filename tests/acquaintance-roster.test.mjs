...
test('all16 referenced targets charge their exact community cost and join correct owned category',()=>{
 assert.equal(WISH_RECRUITS.length,16);assert.equal(WISH_RECRUITS.filter(r=>r.kind==='fellows').length,8);assert.equal(WISH_RECRUITS.filter(r=>r.cost===1).length,5);
