...
test('operation payments affect actual assignment income; paid steps bounded',()=>{let s=act(fresh(0),'openingStart',0).state;s=act(s,'openEnterprise',0,'Building_101').state;s=act(s,'hireEmployees',0,'Building_101',10).state;s=act(s,'recruit',0,'hero_1').state;s=act(s,'assignOperator',0,'Building_101','hero_1').state;s.opening.locker.Item_HeroManagerment_Building=4;const before=totalRate(s);for(let i=0;i<3;i++)s=act(s,'openingOperation',0,'hero_1').state;assert.ok(totalRate(s)>before);assert.equal(openingItemCount(s,'Item_HeroManagerment_Building'),0);assert.ok(act(s,'openingOperation',0,'hero_1').error);});
