...
test('recovered extensionless JSON characters recruit and integrate with work, gifts, bonds and reload',()=>{
 let s=fresh(0);for(const id of ['hero_12','hero_54'])s=act(s,'recruit',0,id).state;s=act(s,'welcome',0,'wife_3').state;s=act(s,'assign',0,'fish','hero_12').state;s=act(s,'bondAssign',0,'wife_3','hero_54').state;s=act(s,'gift',0,'wife_3','gift1').state;assert.equal(s.buildings.fish.fellow,'hero_12');assert.equal(s.bonds.wife_3.fellow,'hero_54');assert.equal(s.family.wife_3.intimacy,1);assert.ok(valid(s));assert.deepEqual(decode(JSON.stringify(s)),s);
});
