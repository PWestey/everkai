...
test('Fifi operation matches type and level 50 Inn bonus without guessing other variants',()=>{
 ...
 assert.equal(fellowOperation(s,'hero_197',inn).known,false);assert.equal(fellowOperation(s,'hero_15',inn).known,false);
 ...
 s.fellows.hero_117.level=60;assert.equal(fellowOperation(s,'hero_117',scroll).percent,150);
});
