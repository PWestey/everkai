import test from 'node:test';import {ripe} from './progression-helpers.mjs';import assert from 'node:assert/strict';import {fresh,act,valid,decode,SAVE_KEY} from '../lib/game.mjs';import {FARM_PLANTS,farmHarvestPlan} from '../lib/farm.mjs';import {createPersistence} from '../lib/persistence.mjs';
...
// step on that plant's LIFETIME harvest record -- 30 of Plant1 for level 1 -> 2, then 5,000 for
// level 2 -> 3 -- and spends nothing (GetPlantUpNoticeById tests upgradeCost <= GetPlantRecordCount).
...
test('legacy growing crops retain level1; unearned, bad levels and full stock are refused',()=>{
 ...
 assert.equal(s.farm.records.Plant1,10,'harvesting records a lifetime tally beside the stock');
 assert.ok(act(s,'sowFarm',1000,0,{plant:'Plant1',level:4}).error);
 // REBASELINED for BUG-41: level 2 must be EARNED (30 Plant1 harvested in all) before it can be
 // sown. It used to be a free pick. One harvest of 10 is not enough; 20 more are needed.
 assert.match(act(s,'sowFarm',1000,0,{plant:'Plant1',level:2}).error,/Reach harvest level 2/);
 assert.match(act(s,'upgradePlant',1000,'Plant1').error,/Harvest 20 more/);
...
test('interrupted higher-level harvest keeps crop and reward together until explicit retry',()=>{
 // REBASELINED for BUG-41: Plant1 level 2 is earned at 30 harvested in all, not bought.
