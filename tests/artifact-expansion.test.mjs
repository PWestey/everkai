...
test('v9 migration preserves progress and seeds only new artifact stacks',()=>{
 let old=run(fresh(1000),'welcome','wife_2').state;old=run(old,'sandboxAdventure').state;old=run(old,'equip','hero_15',club).state;old={...old,artifacts:{...(old.artifacts||{ore:0,bag:{}}),ore:1000}};old=run(old,'upgradeArtifact','hero_15').state;
...
test('expanded artifacts can be claimed, upgraded and transferred without losing copies',()=>{
 assert.equal(GEAR.length,84);assert.equal(added.length,44);let s=run(fresh(1000),'claimAllGear').state;s={...s,artifacts:{...(s.artifacts||{ore:0,bag:{}}),ore:1000}};
...
test('recycling cannot touch upgraded or equipped copies and cannot overflow ore',()=>{
 let s=run(fresh(1000),'claimAllGear').state;s=run(s,'equip','hero_15',club).state;s={...s,artifacts:{...(s.artifacts||{ore:0,bag:{}}),ore:1000}};s=run(s,'upgradeArtifact','hero_15').state;s=run(s,'equip','hero_15',null).state;
