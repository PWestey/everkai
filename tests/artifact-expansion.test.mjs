...
import {basicCopies,artifactState,gearLevel} from '../lib/artifacts.mjs';import added from '../lib/expanded-gear.json' with {type:'json'};import {stockAll,supplied} from './gear-fixtures.mjs';
...
test('v9 migration preserves progress and seeds only new artifact stacks',()=>{
 let old=run(fresh(1000),'welcome','wife_2').state;old=supplied(old);old=run(old,'equip','hero_15',club).state;old={...old,artifacts:{...(old.artifacts||{ore:0,bag:{}}),ore:1000}};old=run(old,'upgradeArtifact','hero_15').state;
...
test('expanded artifacts can be claimed, upgraded and transferred without losing copies',()=>{
 assert.equal(GEAR.length,84);assert.equal(added.length,44);let s=stockAll(fresh(1000));s={...s,artifacts:{...(s.artifacts||{ore:0,bag:{}}),ore:1000}};
 for(const g of added){s=run(s,'equip','hero_15',g.id).state;s={...s,artifacts:{...(s.artifacts||{ore:0,bag:{}}),ore:1000}};s=run(s,'upgradeArtifact','hero_15').state;assert.equal(gearLevel(s.fellows.hero_15),2);s=run(s,'equip','hero_15',null).state;assert.equal(basicCopies(s,g.id),0);assert.equal(s.artifacts.bag[g.id][2],1);}
...
test('recycling cannot touch upgraded or equipped copies and cannot overflow ore',()=>{
 let s=stockAll(fresh(1000));s=run(s,'equip','hero_15',club).state;s={...s,artifacts:{...(s.artifacts||{ore:0,bag:{}}),ore:1000}};s=run(s,'upgradeArtifact','hero_15').state;s=run(s,'equip','hero_15',null).state;
 assert.ok(run(s,'recycleArtifact',club,1).error);s=stockAll(s);const before=s.artifacts.ore;s=run(s,'recycleArtifact',club,'all').state;assert.equal(s.artifacts.ore,before+5);assert.equal(s.inventory[club],1);assert.equal(s.artifacts.bag[club][2],1);
 s=stockAll(s);s.artifacts.ore=1e9-4;assert.ok(run(s,'recycleArtifact',club,1).error);assert.equal(s.inventory[club],2);
...
test('maximum upgrade equals sequential upgrades and preserves unused ore',()=>{
 let s=stockAll(fresh(1000));s=run(s,'equip','hero_15',club).state;s.artifacts.ore=25;
