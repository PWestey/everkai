...
test('supply and Aptitude batches equal singles with exact cost and no cap waste',()=>{
 ...
 s.fellows.hero_15.aptitude=APTITUDE_CAP-1;s.inventory.Item_Talent_Hero_1=25;assert.equal(aptitudeTrainingPlan(s,'hero_15').count,1);assert.equal(run(s,'aptitude','hero_15','max').inventory.Item_Talent_Hero_1,24);s.inventory.Item_Talent_Hero_1=999999;assert.equal(supplyPurchasePlan(s,'Item_Talent_Hero_1',25).count,1);
});
