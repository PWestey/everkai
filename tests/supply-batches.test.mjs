import {PEARL_APTITUDE_CAP} from '../lib/aptitude-cap.mjs'; // direct pearl training stops at 1,000 (the Aptitude cap itself is 31,122)
...
test('supply and Aptitude batches equal singles with exact cost and no cap waste',()=>{
 let s=fresh(1000);s.gold=4999;const p=supplyPurchasePlan(s,'Item_Talent_Hero_1',25);assert.equal(p.count,24);assert.equal(p.cost,4814,'pearls climb the lifetime price ladder: the 10th onward cost 201');let a=run(s,'buySupply','Item_Talent_Hero_1',25),b=s;for(let i=0;i<24;i++)b=run(b,'buySupply','Item_Talent_Hero_1',1);assert.deepEqual(a,b);a=run(a,'aptitude','hero_15','max');for(let i=0;i<24;i++)b=run(b,'aptitude','hero_15',1);assert.deepEqual(a,b);assert.equal(a.gold,185);assert.deepEqual(decode(JSON.stringify(a)),a);
 s.fellows.hero_15.aptitude=PEARL_APTITUDE_CAP-1;s.inventory.Item_Talent_Hero_1=25;assert.equal(aptitudeTrainingPlan(s,'hero_15').count,1);assert.equal(run(s,'aptitude','hero_15','max').inventory.Item_Talent_Hero_1,24);s.inventory.Item_Talent_Hero_1=999999;assert.equal(supplyPurchasePlan(s,'Item_Talent_Hero_1',25).count,1);
});
