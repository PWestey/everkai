...
- Roaming: spend stamina strolling Drakenberg to meet Family members and Fellows; each meeting plays that member's original story, and bonds built through meetings invite new Family members.
...
Artwork derives from the user's supplied Isekai files and selected locally prepared compositions. Original artwork remains owned by its respective rights holders; no ownership or redistribution license is asserted. No original APK, protected bundle, production credential, personal save or private hosting binding is included. The live Spine playback pilot (off unless the page is opened with `?spine=1`) bundles Esoteric Software's Spine Runtimes 4.1 (`@esotericsoftware/spine-webgl`), which are licensed under the Spine Runtimes License in `public/licenses/spine-runtimes-LICENSE.txt`, and serves six original skeleton/atlas exports with humanized, re-encoded pages from `public/assets/spine/`. There is no production game-server connection or authentication bypass.
