...
## Frozen idle clips, measured art framing, roster order and promotion — 16 September 2026

**Frozen clips.** The fitted-framing re-render (a42b4c4) and the two boot fixes produced clips whose frames never change. Measured with `scripts/measure-art-bounds.swift` (fraction of pixels that change across 24 sampled frames): 241 of 516 clips were exactly 0 and 14 more were ≤0.004, against 0.05–0.25 for the clips they replaced. The earlier renders of all 257 re-rendered clips animate. 253 were restored byte-for-byte from `a42b4c4^` (each prior JSON row matched the file's sha256, bytes, size, fps and frame count). Clip total is 756.7 MB again (streamed, not precached). `wife_116` and `wife_19c1` stay frozen: their animated renders still have the misplaced boots, and the render pipeline that fixed them no longer exists. `tests/frozen-idle-clips.json` blocks the frozen bytes from returning. The frozen renders did have tighter framing, which the next part replaces.

**Framing by measured bounds.** 264 of 523 stills and 260 of 516 clips draw the art inside a flat viewer surround, often small, low or off-centre (up to 17% of frame width off-centre for W13C1). `lib/art-bounds-data.json` records each one's exact art bounds (every pixel differing from the surround; clips take the union over 24 frames). Cross-checked against an independent sharp decode on all 264 surround stills: same set, bounds within 3% for 259 and within 7% for the rest (soft shadows at the threshold). The character screen, Art dialog and Family art stage zoom and centre surround renders to their art, never showing it smaller than before, with the surround colour painted behind (`app/art-framing.tsx`). Roster cards place surround renders so the art stands as tall as a full-frame figure, with its top under the arch (`rosterArtPlacement`). Renders with scenery backgrounds have no measurable surround, so they keep their placement. Re-run `node scripts/measure-art-bounds.mjs` (macOS) after changing character art. A test fails if any shown asset is unmeasured.

**Roster order.** Joined Fellows are sorted by Power (`bondedPower`, the number on the character screen), strongest first. Joined Family are sorted by Blessing Power. Characters not yet joined follow in catalogue order. The character screen's arrows step through the same order (`lib/roster-filter.mjs`).

**Promotion from the profile.** The level badge's profile shows Fame and the earnings requirement, with Promote (`openingPromote`) and buttons to Stages, Roaming, Habits and Businesses. `startingSave()` creates no `opening` subtree; it appears only when the player taps Start journey. Until then, habit Fame was zeroed and roaming Fame was dropped, even though roaming's message still said "Fame +N". Both now pay into a newly opened `freshOpening()`, the same state `openingStart` writes, so no save changes shape.
